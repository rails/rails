...
class AssertSelectEmailTest < ActionMailer::TestCase
  ...
  def test_assert_part_last_mail_delivery
    ...
    assert_part :html do |part|
      assert_includes part.body, "<div><p>foo</p><p>bar</p></div>"
    end
    assert_decoded_part :html do |html|
...
class AssertMultipartSelectEmailTest < ActionMailer::TestCase
  ...
  def test_assert_part_last_mail_delivery
    ...
    assert_part :text do |part|
      assert_includes part.body, "foo bar"
    end
    assert_decoded_part :text do |text|
  ...
  def test_assert_decoded_part_html_without_block
    assert_decoded_part :html, AssertMultipartSelectMailer.test(html: "html")

    assert_raises Minitest::Assertion, match: "expected part matching text/html" do
      assert_decoded_part :html, AssertMultipartSelectMailer.test(text: "text")
    end
  end

  def test_assert_decoded_part_text_without_block
    assert_decoded_part :text, AssertMultipartSelectMailer.test(text: "text")

    assert_raises Minitest::Assertion, match: "expected part matching text/plain" do
      assert_decoded_part :text, AssertMultipartSelectMailer.test(html: "html")
    end
  end
