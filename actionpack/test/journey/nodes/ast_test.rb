  ...
  module Journey
    module Nodes
      class TestAst < ActiveSupport::TestCase
        ...
        def test_default_regexp_is_true
          tree = Journey::Parser.new.parse("/:path/:symbol")
          ast = Ast.new(tree, true)

          assert_predicate ast, :default_regexp?
        end

        def test_default_regexp_is_false
          tree = Journey::Parser.new.parse("/:path/*glob")
          ast = Ast.new(tree, true)

          assert_not_predicate ast, :default_regexp?
        end
