...
module ActionDispatch
  module Routing
    class TestRoute < ActiveSupport::TestCase
      def test_initialize
        app      = Object.new
        path     = Path.new '/:controller(/:action(/:id(.:format)))'
        defaults = {}
      ...
      def test_ip_address
        path  = Path.new '/messages/:id(.:format)'
      ...
      def test_format_with_star
        path  = Path.new '/:controller/*extra'
        route = Route.new("name", nil, path, {},
                          { :controller => 'foo', :action => 'bar' })
      ...
      def test_extras_are_not_included_if_optional_with_parameter
        path  = Path.new '(/sections/:section)/pages/:id'
        route = Route.new("name", nil, path, { }, { :action => 'show' })
      ...
      def test_score
        ...
        path = Path.new "/page/:id(/:action)(.:format)"
