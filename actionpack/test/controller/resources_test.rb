...
class ResourcesTest < ActionController::TestCase
  ...
  def test_invalid_only_option_for_resources_api_only
    expected_message = ":only and :except must include only [:index, :create, :show, :update, :destroy], but also included [:new]"
    assert_raise(ArgumentError, match: expected_message) do
      with_routing(ActionDispatch::Routing::RouteSet::Config.new(nil, true)) do |set|
        set.draw do
