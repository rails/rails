...
class WebServiceTest < ActionDispatch::IntegrationTest
  ...
  def test_register_and_use_json_simple
    with_test_route_set do
      with_params_parsers ActiveSupport::Mime[:json] => Proc.new { |data| ActiveSupport::JSON.decode(data)['request'].with_indifferent_access } do
        post "/",
  ...
  def test_parsing_json_doesnot_rescue_exception
    req = Class.new(ActionDispatch::Request) do
      def params_parsers
        { ActiveSupport::Mime[:json] => Proc.new { |data| raise Interrupt } }
      end
