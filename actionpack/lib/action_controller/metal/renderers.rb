...
module ActionController
  ...
  module Renderers
    ...
    #
    #   ActionController.add_serializer :csv do |obj, options|
    ...
    # Serializers define a function called within a renderer specific to
    # transforming the object into a mime-compatible type.
    # See <tt>Renderers.add</tt>
    #
    # The separation of serialization from rendering allows
    # customizing the response body without changing other aspects of rendering
    # a response in the desired format, such as setting content type, handling
    # callbacks, etc, rather than, for example, requiring one to define a
    # method +_render_with_renderer_json+ in a subclass and optionally call super on it.
    #
    ...
    # in the controller, and calling +super+ on the serialized object.
    # Now, one need only call +ActionController.remove_serializer :json+ and
    ...
    #
    #   ActionController.add_serializer :json do |json, options|
    ...
    def _render_to_body_with_renderer(options)
      _renderers.each do |renderer_format|
        next unless options.key?(renderer_format)
        _process_options(options)
        renderer_method_name = Renderers._render_with_renderer_method_name(renderer_format)
        value_to_render = options.delete(renderer_format)
