...
module ActiveStorage::InMemoryBackend
  class Blob
    ...
    self.storage_attributes += %i[key filename content_type byte_size checksum metadata service_name]
    ...
    def attributes
      self.class.storage_attributes.to_h { |attribute| [attribute.to_s, public_send(attribute)] }
    end
    ...
    def identify_without_saving
      ...
      chunk = byte_size.positive? ? download_chunk(0...4.kilobytes) : ""
      self.content_type = Marcel::MimeType.for(chunk, name: filename.to_s, declared_type: content_type)
    ...
    def service_metadata
      { content_type: content_type_for_serving, custom_metadata: custom_metadata }.tap do |metadata|
        if disposition = forced_disposition_for_serving
          metadata.merge!(disposition: disposition, filename: filename)
        end
      end
    end
    ...
    private
      def attributes_for_persistence
        super.merge(filename: filename.to_s)
      end
