...
module ActiveStorage::InMemoryBackend
  class Relation
    ...
    def where(attributes = nil)
      if attributes
        self.class.new(@model, @filters + [ cast_ids(attributes) ], @exclusions)
      else
        WhereChain.new(self)
      end
    end
    ...
    def not(attributes)
      self.class.new(@model, @filters, @exclusions + [ cast_ids(attributes) ])
    end
    ...
    private
      # Global ID looks records up with `where(id: ids)` and passes the ids as
      # strings, while the store keys records by integer. Cast the primary key
      # the way `find` does. Other id attributes, such as an owner's record_id,
      # are left alone because they are not necessarily integers.
      def cast_ids(attributes)
        attributes.to_h do |key, value|
          if key.to_s == "id"
            [ key, value.is_a?(Array) ? value.map { |id| cast_id(id) } : cast_id(value) ]
          else
            [ key, value ]
          end
        end
