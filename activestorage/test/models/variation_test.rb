...
class ActiveStorage::VariationTest < ActiveSupport::TestCase
  ...
  test "variations have the same digest when decoded from their signed key" do
    variation = ActiveStorage::Variation.new(resize_to_limit: [100, 100], format: :webp)

    assert_equal variation.digest, ActiveStorage::Variation.decode(variation.key).digest
  end
