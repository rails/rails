...
class ActiveStorage::ActiveModelOwnerAttachedTest < ActiveSupport::TestCase
  ...
  test "owner overrides can call generated attachment methods with super" do
    @owner_class.class_eval do
      def avatar=(attachable)
        super(attachable.merge(filename: "renamed.txt"))
      end

      def photos=(attachables)
        super(attachables.reverse)
      end
    ...
    owner.photos = [lifecycle_upload("first"), lifecycle_upload("second")]
    owner.save!

    assert_equal "renamed.txt", owner.avatar.filename.to_s
    assert_equal ["second", "first"], owner.photos.blobs.map(&:download)
    assert_equal 2, owner.photos_attachments.size
  end

  test "backend reads isolate unsaved attributes and nested metadata" do
    blob = create_memory_blob
    blob.metadata[:custom] = { "labels" => [ "original" ] }
    blob.save!
    blob.metadata[:custom]["labels"] << "unsaved"

    stored_blob = ActiveStorage.blob_class.find(blob.id)
    assert_not_same blob, stored_blob
    assert_equal [ "original" ], stored_blob.metadata[:custom]["labels"]

    stored_blob.metadata[:custom]["labels"] << "also unsaved"
    assert_equal [ "original" ], ActiveStorage.blob_class.records.first.metadata[:custom]["labels"]
    assert_equal [ "original" ], blob.reload.metadata[:custom]["labels"]
  end

  test "owner reads isolate unsaved attributes and attachment changes" do
    owner = @owner_class.new(name: "Dorian")
    owner.save!
    owner.name = "Basil"
    owner.avatar = { io: StringIO.new("pending"), filename: "pending.txt" }

    stored_owner = @owner_class.find(owner.id)
    assert_not_same owner, stored_owner
    assert_equal "Dorian", stored_owner.name
    assert_empty stored_owner.attachment_changes
    assert_not stored_owner.avatar.attached?
    assert_not stored_owner.changed?

    owner.reload
    assert_equal "Dorian", owner.name
  ...
  test "requires public owner identity readers" do
    %i[id persisted?].each do |method|
      owner_class = Class.new(@owner_class) { private method }

      error = assert_raises(ActiveStorage::OwnerContractMissing) { owner_class.has_one_attached :file }
      assert_includes error.message, method.to_s
    end
  end

  test "requires a named owner class" do
    error = assert_raises(ActiveStorage::OwnerContractMissing) do
      Class.new(@owner_class).has_one_attached :file
    end

    assert_includes error.message, "class name"
  end
  ...
  test "refuses active model owners with default active record storage classes" do
    ...
    ActiveStorage.attachment_class = "ActiveStorage::Attachment"
    ActiveStorage.variant_record_class = "ActiveStorage::VariantRecord"
  ...
  test "validates named services after services registry is loaded" do
    ...
    ActiveStorage::Services.setup_from_app_config(Rails.application)
    assert_equal :local, owner_class.reflect_on_attachment(:avatar).options[:service_name]
  end
  ...
  test "service setup validates declarations made before configuration" do
    ActiveStorage::Services.registry = nil
    @owner_class.has_one_attached :missing_service, service: :unconfigured

    error = assert_raises(ArgumentError) { ActiveStorage::Services.setup_from_app_config(Rails.application) }
    assert_includes error.message, "ActiveModelOwner#missing_service"
    assert_includes error.message, ":unconfigured"
  end

  test "service setup requires a default for declarations without a named service" do
    Rails.configuration.active_storage.with(service: nil) do
      error = assert_raises(RuntimeError) { ActiveStorage::Services.setup_from_app_config(Rails.application) }
      assert_includes error.message, "Missing Active Storage service name"
    end
  end

  test "service validation does not expose instance attributes on active record models" do
    assert_not ActiveStorage::Blob.method_defined?(:pending_service_validations)
    assert_not ActiveStorage::Blob.method_defined?(:pending_service_validations=)
  end
  ...
  test "failed generic replacement does not purge stale dependent blob" do
    ...
    assert_raises(ActiveStorage::RecordNotSaved) { change.save }
    ActiveStorage::Attached::Changes.flush_pending_purges(change.deferred_purges)
  ...
  test "failed attachment cleanup still cleans up newly saved blobs and preserves existing blobs" do
    [false, true].each do |existing_blob|
      owner = @owner_class.new(name: "Dorian")
      owner.save!
      owner.avatar = existing_blob ? create_memory_blob : lifecycle_upload("pending")
      change = owner.attachment_changes.fetch("avatar")
      save_error = ActiveStorage::RecordNotSaved.new("attachment save failed", change.attachment)
      change.attachment.define_singleton_method(:save!) do
        super()
        raise save_error
      end
      change.attachment.define_singleton_method(:destroy) do
        super()
        raise "attachment cleanup failed"
      end

      error = assert_raises(ActiveStorage::RecordNotSaved) { owner.save! }

      assert_same save_error, error
      assert_not change.attachment.persisted?
      assert_equal existing_blob, change.blob.persisted?
      assert_equal existing_blob, change.blob.service.exist?(change.blob.key)
    end
  end
  ...
  test "attachment and blob collections support any with patterns and blocks" do
    owner = @owner_class.new(name: "Dorian")
    owner.photos = [create_memory_blob]
    owner.save!

    [
      [owner.photos_attachments, ActiveStorage.attachment_class],
      [owner.photos_blobs, ActiveStorage.blob_class]
    ].each do |collection, type|
      assert collection.any?(type)
      assert_not collection.any?(String)
      assert collection.any?(&:persisted?)
    end
  end
  ...
  test "has many attaches appends and exposes collection helpers" do
    ...
    assert_equal first_blob.id, owner.photos_blobs.find { |blob| blob.id == first_blob.id }.id
    assert_raises(ActiveStorage::QueryNotSupported) { owner.photos_attachments.find(first_blob.id) }
    assert_raises(ActiveStorage::QueryNotSupported) { owner.photos_blobs.find(first_blob.id) }
  ...
  test "has many attachments use id as secondary order for matching timestamps" do
    ...
    store = ActiveStorage.attachment_class.store
    records = store.each_pair.to_a.reverse
    store.clear
    records.each { |id, attributes| store[id] = attributes }
  ...
  test "restored attachments protect blobs when an unreported rollback is followed by a save" do
    owner = @owner_class.new(name: "Dorian")
    owner.save!
    blob = create_memory_blob(filename: "funky.jpg")
    ...
    # The backend must protect referenced blobs even when it fails to notify
    # Active Storage that the transaction rolled back.
  ...
  test "restored attachments protect blobs when an unreported rollback is followed by a commit" do
    owner = @owner_class.new(name: "Dorian")
    owner.save!
    blob = create_memory_blob(filename: "funky.jpg")
    owner.avatar.attach(blob) # default dependent: :purge_later
    ...
    # A standalone commit must not purge blobs referenced by the restored rows.
    perform_enqueued_jobs only: ActiveStorage::PurgeJob do
      owner.send(:run_callbacks, :commit) { true }
    end
    assert blob.persisted?
  ...
  test "owner without commit callbacks purges a blob shared across attachments once all rows are gone" do
    ...
    # The single destroy handler visits reflections in declaration order.
    # Defer the purge until the later dependent: false reference is also removed.
  ...
  test "generic destroy callback applies dependent purge without an attachment hook" do
    ...
    ActiveStorage.attachment_class = "ActiveStorage::InMemoryBackend::AttachmentWithoutDependentPurge"
  ...
  test "failed stale generic attachment destroy cleans up newly saved has one records" do
    ...
    ActiveStorage.attachment_class = "ActiveStorage::InMemoryBackend::AttachmentWithStaleDestroyFailure"
  ...
  test "halted generic attachment destroy raises instead of silently continuing" do
    ...
    ActiveStorage.attachment_class = "ActiveStorage::InMemoryBackend::AttachmentWithHaltedDestroy"
  ...
  test "generic owner analysis modes" do
    ...
    assert later_blob.reload.analyzed?
  ...
  test "generic lazy analysis persists direct upload identification" do
    owner = @owner_class.new(name: "Dorian")
    blob = create_memory_direct_upload

    assert_no_enqueued_jobs only: ActiveStorage::AnalyzeJob do
      owner.avatar_with_lazy_analysis = blob
      owner.save!
    end

    stored_blob = ActiveStorage.blob_class.find(blob.id)
    assert_equal "image/jpeg", stored_blob.content_type
    assert stored_blob.identified?
    assert_not stored_blob.analyzed?
  end

  test "generic lazy analysis persists signed direct upload identification" do
    owner = @owner_class.new(name: "Dorian")
    blob = create_memory_direct_upload

    owner.avatar_with_lazy_analysis = blob.signed_id
    owner.save!
  ...
  test "replacing a has many assignment preserves direct upload identification" do
    @owner_class.has_many_attached :lazy_photos, analyze: :lazily
    owner = @owner_class.new(name: "Dorian")
    blob = create_memory_direct_upload

    owner.lazy_photos = [ blob ]
    assert blob.identified?
    assert_not ActiveStorage.blob_class.find(blob.id).identified?
    owner.lazy_photos = [ blob ]
    owner.save!
