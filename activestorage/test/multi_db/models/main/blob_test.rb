...
class ActiveStorage::MainBlobTest < ActiveSupport::TestCase
  ...
  test "unattached scope" do
    [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ].tap do |blobs|
      MainUser.create! name: "DHH", avatar: blobs.first
      assert_includes ActiveStorage::MainBlob.unattached, blobs.second
      assert_not_includes ActiveStorage::MainBlob.unattached, blobs.first

      MainUser.create! name: "Jason", avatar: blobs.second
      assert_not_includes ActiveStorage::MainBlob.unattached, blobs.second
    end
  end

  test "create_and_upload does not permit a conflicting blob key to overwrite an existing object" do
    data = "First file"
    blob = create_main_blob data: data

    assert_raises ActiveRecord::RecordNotUnique do
      ActiveStorage::MainBlob.stub :generate_unique_secure_token, blob.key do
        create_main_blob data: "This would overwrite"
      end
    end
  ...
  test "create_and_upload accepts a record for overrides" do
    assert_nothing_raised do
      create_main_blob(record: MainUser.new)
    end
  end

  test "create_and_upload raises for non-rewindable io" do
    assert_raises(ArgumentError) do
      ActiveStorage::MainBlob.create_and_upload!(io: file_fixture("racecar.jpg"), filename: "racecar.jpg")
    end
  end
  ...
  test "compose" do
    blobs = 3.times.map { create_main_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false) }
    blob = ActiveStorage::MainBlob.compose(blobs, filename: "all_numbers.txt")
  ...
  test "compose with unpersisted blobs" do
    blobs = 3.times.map { create_main_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false).dup }

    error = assert_raises(ActiveRecord::RecordNotSaved) do
      ActiveStorage::MainBlob.compose(blobs, filename: "all_numbers.txt")
    end
  ...
  test "compose with custom key" do
    blobs = 3.times.map { create_main_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false) }
    blob = ActiveStorage::MainBlob.compose(blobs, key: "custom_key", filename: "all_numbers.txt")
  ...
  test "text?" do
    blob = create_main_blob data: "Hello world!"
    assert_predicate blob, :text?
    assert_not_predicate blob, :audio?
  end

  test "download yields chunks" do
    blob   = create_main_blob data: "a" * 5.0625.megabytes
    chunks = []
  ...
  test "open with integrity" do
    create_main_file_blob(filename: "racecar.jpg").tap do |blob|
      blob.open do |file|
  ...
  test "open without integrity" do
    create_main_blob(data: "Hello, world!").tap do |blob|
  ...
  test "open in a custom tmpdir" do
    create_main_file_blob(filename: "racecar.jpg").open(tmpdir: tmpdir = Dir.mktmpdir) do |file|
      assert_predicate file, :binmode?
      assert_equal 0, file.pos
  ...
  test "URLs force content_type to binary and attachment as content disposition for content types served as binary" do
    blob = create_main_blob(content_type: "text/html")
  ...
  test "URLs force attachment as content disposition when the content type is not allowed inline" do
    blob = create_main_blob(content_type: "application/zip")
  ...
  test "URLs allow for custom filename" do
    blob = create_main_blob(filename: "original.txt")
  ...
  test "purge does nothing when attachments exist" do
    create_main_blob.tap do |blob|
      MainUser.create! name: "DHH", avatar: blob
      assert_no_difference(-> { ActiveStorage::MainBlob.count }) { blob.purge }
  ...
  test "uses service from blob when provided" do
    with_main_service("mirror") do
      blob = create_main_blob(filename: "funky.jpg", service_name: :local)
  ...
  test "scope_for_strict_loading adds includes only when track_variants and strict_loading_by_default" do
    assert_empty ActiveStorage::MainBlob.scope_for_strict_loading.includes_values

    with_strict_loading_by_default do
      assert_not_empty ActiveStorage::MainBlob.scope_for_strict_loading.includes_values
  ...
  private
    def expected_url_for(blob, disposition: :attachment, filename: nil, content_type: nil, service_name: :local)
      ...
      "https://example.com/rails/active_storage/main/disk/#{ActiveStorage.verifier.generate(key_params, expires_in: 5.minutes, purpose: :blob_key)}/#{filename}"
    end
end
