...
class ActiveStorage::MainAttachmentTest < ActiveSupport::TestCase
  include ActiveJob::TestHelper

  setup do
    @user = MainUser.create!(name: "Josh")
  end

  teardown { ActiveStorage::MainBlob.all.each(&:delete) }

  test "analyzing a directly-uploaded blob after attaching it" do
    blob = directly_upload_main_file_blob(filename: "racecar.jpg")
  ...
  test "attaching a un-analyzable blob" do
    blob = create_main_blob(filename: "blank.txt")
  ...
  test "attaching a blob doesn't touch the record" do
    ...
    blob = create_main_blob_before_direct_upload byte_size: data.size, checksum: ActiveStorage.checksum_implementation.base64digest(data)
    blob.upload(io)
  ...
  test "mirroring a directly-uploaded blob after attaching it" do
    with_main_service("mirror") do
      blob = directly_upload_main_file_blob
      assert_not ActiveStorage::MainBlob.service.mirrors.second.exist?(blob.key)

      perform_enqueued_jobs do
        @user.highlights.attach(blob)
      end

      assert ActiveStorage::MainBlob.service.mirrors.second.exist?(blob.key)
    end
  end

  test "directly-uploaded blob identification for one attached occurs before validation" do
    blob = directly_upload_main_file_blob(filename: "racecar.jpg", content_type: "application/octet-stream")
  ...
  test "getting a signed blob ID from an attachment" do
    blob = create_main_blob
    @user.avatar.attach(blob)

    signed_id = @user.avatar.signed_id
    assert_equal blob, ActiveStorage::MainBlob.find_signed!(signed_id)
    assert_equal blob, ActiveStorage::MainBlob.find_signed(signed_id)
  end
  ...
  test "getting a signed blob ID from an attachment with a custom purpose" do
    ...
    assert_equal blob, ActiveStorage::MainBlob.find_signed!(signed_id, purpose: :custom_purpose)
  end
  ...
  test "fail to find blob within expiration duration" do
    ...
    signed_id = @user.avatar.signed_id(expires_in: 1.minute)
    travel 2.minutes
    assert_nil ActiveStorage::MainBlob.find_signed(signed_id)
  end
  ...
  test "signed blob ID backwards compatibility" do
    ...
    assert_equal blob, ActiveStorage::MainBlob.find_signed!(signed_id_generated_old_way)
  end
  ...
  test "can create an attachment with the record having no attachment reflections" do
    assert_nothing_raised { ActiveStorage::MainAttachment.create!(name: "whatever", record: @user, blob: create_main_blob) }
  end
  ...
  private
    ...
    def assert_blob_identified_outside_transaction(blob, &block)
      baseline_transaction_depth = MainRecord.lease_connection.open_transactions
      max_transaction_depth = -1

      track_transaction_depth = ->(*) do
        max_transaction_depth = [MainRecord.lease_connection.open_transactions, max_transaction_depth].max
      end
