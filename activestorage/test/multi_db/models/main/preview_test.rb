...
class ActiveStorage::MainPreviewTest < ActiveSupport::TestCase
  ...
  test "preview of PDF is created on the same service" do
    blob = create_main_file_blob(filename: "report.pdf", content_type: "application/pdf", service_name: "local_public")
    preview = blob.preview(resize_to_limit: [640, 280]).processed
  ...
  test "preview of MP4 video is created on the same service" do
    blob = create_main_file_blob(filename: "video.mp4", content_type: "video/mp4", service_name: "local_public")
    preview = blob.preview(resize_to_limit: [640, 280]).processed
