...
class ActiveStorage::MainVariantWithRecordTest < ActiveSupport::TestCase
  ...
  test "variant of a blob is on the same service" do
    blob = create_main_file_blob(filename: "racecar.jpg", service_name: "local_public")
  ...
  test "eager loading has_one_attached record" do
    user1 = MainUser.create!(name: "Josh")
    user2 = MainUser.create!(name: "John")

    blob1 = directly_upload_main_file_blob(filename: "racecar.jpg")
    assert_difference -> { ActiveStorage::MainVariantRecord.count }, +1 do
      blob1.representation(resize_to_limit: [100, 100]).processed
    end

    blob2 = directly_upload_main_file_blob(filename: "racecar_rotated.jpg")
    assert_difference -> { ActiveStorage::MainVariantRecord.count }, +1 do
      blob2.representation(resize_to_limit: [100, 100]).processed
    end

    assert_no_difference -> { ActiveStorage::MainVariantRecord.count } do
      user1.cover_photo.attach(blob1)
      user2.cover_photo.attach(blob2)
    end

    users = MainUser.where(id: [user1.id, user2.id])
  ...
  def find_keys_for_representation(filename)
    user = MainUser.create!(name: "Justin")

    10.times do
      blob = directly_upload_main_file_blob(filename: filename)
    ...
    highlights = MainUser.with_attached_highlights_with_variants.find_by(name: "Justin").highlights_with_variants.to_a
  ...
  def test_no_n_plus_1_with_all_variant_records_on_attached_video
    ...
    10.times do
      blob = directly_upload_main_file_blob(filename: "video.mp4")
      user.highlights_with_variants.attach(blob)
    end
  ...
  test "eager loading has_many_attached records" do
    ...
    assert_no_difference -> { ActiveStorage::MainVariantRecord.count } do
      assert_queries_count(7) do
        ...
        # attachments for the variant records x 1
        # blobs for the attachments for the variant records x 1
        MainUser.where(id: user.id).with_attached_vlogs.each do |u|
          u.vlogs.map do |vlog|
            rep = vlog.representation(resize_to_limit: [100, 100])
            rep.processed
            rep.key
    ...
    assert_difference -> { ActiveStorage::MainVariantRecord.count }, +2 do
  ...
  test "destroy deletes file from service" do
    ...
    assert_equal 1, ActiveStorage::MainVariantRecord.count
    assert blob.service.exist?(variant.key)

    variant.destroy

    assert_equal 0, ActiveStorage::MainVariantRecord.count
