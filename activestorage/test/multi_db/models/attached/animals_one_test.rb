...
class ActiveStorage::OneAttachedAnimalsTest < ActiveSupport::TestCase
  ...
  test "creating a record with a File as attachable attribute" do
    @user = AnimalsUser.create!(name: "Dorian", avatar: file_fixture("image.gif").open)
  ...
  test "creating a record with a Pathname as attachable attribute" do
    @user = AnimalsUser.create!(name: "Dorian", avatar: file_fixture("image.gif"))
  ...
  test "attaching an existing blob to an existing record" do
    @user.avatar.attach create_animals_blob(filename: "funky.jpg")
  ...
  test "attaching an existing blob from a signed ID to an existing record" do
    @user.avatar.attach create_animals_blob(filename: "funky.jpg").signed_id
    assert_equal "funky.jpg", @user.avatar.filename.to_s
  end

  test "attaching an existing blob from a signed ID passes record" do
    blob = create_animals_blob(filename: "funky.jpg")

    assert_called_with(ActiveStorage::AnimalsBlob, :find_signed!, [blob.signed_id], returns: blob, record: @user) do
  ...
  test "attaching a new blob from a Hash to an existing record passes record" do
    ...
    blob = ActiveStorage::AnimalsBlob.build_after_unfurling(**hash)
    arguments = hash.merge(record: @user, service_name: nil)
    assert_called_with(ActiveStorage::AnimalsBlob, :build_after_unfurling, [], **arguments, returns: blob) do
  ...
  test "attaching a new blob from an uploaded file to an existing record passes record" do
    ...
    blob = ActiveStorage::AnimalsBlob.build_after_unfurling(**arguments)
    assert_called_with(ActiveStorage::AnimalsBlob, :build_after_unfurling, [], returns: blob, **arguments) do
      @user.avatar.attach upload
    end
  end

  test "attaching a blob to a persisted, unchanged, and valid record, returns the attachment" do
    return_value = @user.avatar.attach create_animals_blob(filename: "funky.jpg")
  ...
  test "updating an existing record to attach an existing blob" do
    @user.update! avatar: create_animals_blob(filename: "funky.jpg")
    assert_equal "funky.jpg", @user.avatar.filename.to_s
  end

  test "updating an existing record to attach an existing blob from a signed ID" do
    @user.update! avatar: create_animals_blob(filename: "funky.jpg").signed_id
    assert_equal "funky.jpg", @user.avatar.filename.to_s
  end
  ...
  test "successfully updating an existing record to attach a new blob from an uploaded file" do
    ...
    assert_equal "racecar.jpg", @user.avatar.filename.to_s
    assert_not ActiveStorage::AnimalsBlob.service.exist?(@user.avatar.key)

    @user.save!
    assert ActiveStorage::AnimalsBlob.service.exist?(@user.avatar.key)
  end
  ...
  test "successfully replacing an existing, dependent attachment on an existing record" do
    create_animals_blob(filename: "funky.jpg").tap do |old_blob|
      @user.avatar.attach old_blob

      perform_enqueued_jobs do
        @user.avatar.attach create_animals_blob(filename: "town.jpg")
      end

      assert_equal "town.jpg", @user.avatar.filename.to_s
      assert_not ActiveStorage::AnimalsBlob.exists?(old_blob.id)
      assert_not ActiveStorage::AnimalsBlob.service.exist?(old_blob.key)
    end
  end

  test "replacing an existing, independent attachment on an existing record" do
    @user.cover_photo.attach create_animals_blob(filename: "funky.jpg")

    assert_no_enqueued_jobs only: ActiveStorage::PurgeJob do
      @user.cover_photo.attach create_animals_blob(filename: "town.jpg")
    end
  ...
  test "replacing an attached blob on an existing record with itself" do
    create_animals_blob(filename: "funky.jpg").tap do |blob|
  ...
  test "successfully updating an existing record to replace an existing, dependent attachment" do
    create_animals_blob(filename: "funky.jpg").tap do |old_blob|
      ...
      perform_enqueued_jobs do
        @user.update! avatar: create_animals_blob(filename: "town.jpg")
      end
  ...
  test "successfully updating an existing record to replace an existing, independent attachment" do
    ...
    assert_no_enqueued_jobs only: ActiveStorage::PurgeJob do
      @user.update! cover_photo: create_animals_blob(filename: "town.jpg")
    end
  ...
  test "analyzing a directly-uploaded blob after attaching it to an existing record" do
    perform_enqueued_jobs do
      @user.avatar.attach directly_upload_animals_file_blob(filename: "racecar.jpg")
    end
  ...
  test "analyzing a directly-uploaded blob after attaching it to an existing record via updates" do
    perform_enqueued_jobs do
      @user.update! avatar: directly_upload_animals_file_blob(filename: "racecar.jpg")
    end
  ...
  test "creating an attachment as part of an autosave association through nested attributes" do
    group = AnimalsGroup.create!(users_attributes: [{ name: "John", avatar: { io: StringIO.new("STUFF"), filename: "town.jpg", content_type: "image/jpeg" } }])
    group.save!
    new_user = AnimalsUser.find_by(name: "John")
    assert_predicate new_user.avatar, :attached?
  end

  test "updating an attachment as part of an autosave association" do
    group = AnimalsGroup.create!(users: [@user])
    @user.avatar = fixture_file_upload("racecar.jpg")
    group.save!
    @user.reload
  ...
  test "creating a record with an existing blob attached" do
    user = AnimalsUser.create!(name: "Jason", avatar: create_animals_blob(filename: "funky.jpg"))
    assert_equal "funky.jpg", user.reload.avatar.filename.to_s
  end

  test "creating a record with an existing blob from a signed ID attached" do
    user = AnimalsUser.create!(name: "Jason", avatar: create_animals_blob(filename: "funky.jpg").signed_id)
    assert_equal "funky.jpg", user.reload.avatar.filename.to_s
  end

  test "creating a record with a new blob from an uploaded file attached" do
    AnimalsUser.new(name: "Jason", avatar: fixture_file_upload("racecar.jpg")).tap do |user|
      assert_predicate user, :new_record?
      assert_predicate user.avatar.attachment, :new_record?
      assert_predicate user.avatar.blob, :new_record?
      assert_equal "racecar.jpg", user.avatar.filename.to_s
  ...
  test "creating a record with an unexpected object attached" do
    error = assert_raises(ArgumentError) { AnimalsUser.create!(name: "Jason", avatar: :foo) }
  ...
  test "analyzing a new blob from an uploaded file after attaching it to a new record" do
    perform_enqueued_jobs do
      user = AnimalsUser.create!(name: "Jason", avatar: fixture_file_upload("racecar.jpg"))
      assert_predicate user.avatar.reload, :analyzed?
      assert_equal 4104, user.avatar.metadata[:width]
      assert_equal 2736, user.avatar.metadata[:height]
    end
  ...
  test "analyzing a directly-uploaded blob after attaching it to a new record" do
    perform_enqueued_jobs do
      user = AnimalsUser.create!(name: "Jason", avatar: directly_upload_animals_file_blob(filename: "racecar.jpg"))
      assert_predicate user.avatar.reload, :analyzed?
      assert_equal 4104, user.avatar.metadata[:width]
      assert_equal 2736, user.avatar.metadata[:height]
    end
  ...
  test "detaching" do
    create_animals_blob(filename: "funky.jpg").tap do |blob|
      ...
      assert_not @user.avatar.attached?
      assert ActiveStorage::AnimalsBlob.exists?(blob.id)
      assert ActiveStorage::AnimalsBlob.service.exist?(blob.key)
    end
  end
  ...
  test "duped record does not share attachment changes" do
    ...
    @user.dup.avatar.attach create_animals_blob(filename: "town.jpg")
    assert_not_predicate @user, :changed_for_autosave?
  end

  test "clearing change on reload" do
    @user.avatar = create_animals_blob(filename: "funky.jpg")
    assert_predicate @user.avatar, :attached?
  ...
  test "overriding attached reader" do
    ...
    begin
      ...
      AnimalsUser.remove_method :avatar
    end
  end
  ...
  test "raises error when misconfigured service is passed" do
    ...
    assert_match(/Cannot configure service :unknown for AnimalsUser#featured_photo/, error.message)
  end
  ...
  test "raises error when unknown variant name is used to generate variant" do
    ...
    assert_match(/Cannot find variant :unknown for AnimalsUser#avatar_with_variants/, error.message)
  end
  ...
  test "transforms variants later conditionally via proc" do
    assert_no_enqueued_jobs only: [ ActiveStorage::TransformJob, ActiveStorage::PreviewImageJob ] do
      @user.avatar_with_conditional_preprocessed.attach create_animals_file_blob(filename: "racecar.jpg")
    end
  ...
  test "avoids enqueuing transform later job or preview later job if there aren't any variants to preprocess" do
    blob = create_animals_file_blob(filename: "report.pdf")
