...
class ActiveStorage::ManyAttachedMainTest < ActiveSupport::TestCase
  ...
  test "attaching existing blobs to an existing record" do
    @user.highlights.attach create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg")
  ...
  test "attaching existing blobs from signed IDs to an existing record" do
    @user.highlights.attach create_main_blob(filename: "funky.jpg").signed_id, create_main_blob(filename: "town.jpg").signed_id
    assert_equal "funky.jpg", @user.highlights.first.filename.to_s
    assert_equal "town.jpg", @user.highlights.second.filename.to_s
  end
  ...
  test "attaching new blobs from uploaded files to an existing, changed record one at a time" do
    ...
    assert_not @user.highlights.second.persisted?
    assert_predicate @user, :will_save_change_to_name?
    assert_not ActiveStorage::MainBlob.service.exist?(@user.highlights.first.key)
    assert_not ActiveStorage::MainBlob.service.exist?(@user.highlights.second.key)

    @user.save!
    assert_equal "racecar.jpg", @user.highlights.reload.first.filename.to_s
    assert_equal "video.mp4", @user.highlights.second.filename.to_s
    assert ActiveStorage::MainBlob.service.exist?(@user.highlights.first.key)
    assert ActiveStorage::MainBlob.service.exist?(@user.highlights.second.key)
  end
  ...
  test "attaching new blobs within a transaction uploads all the files" do
    ...
    assert ActiveStorage::MainBlob.service.exist?(@user.highlights.first.key)
    assert ActiveStorage::MainBlob.service.exist?(@user.highlights.second.key)
    assert ActiveStorage::MainBlob.service.exist?(@user.highlights.third.key)
  end
  ...
  test "attaching many new blobs within a transaction on a new record uploads all the files" do
    user = MainUser.create!(name: "John") do |user|
  ...
  test "attaching new blobs within a transaction create the exact amount of records" do
    assert_difference -> { ActiveStorage::MainBlob.count }, +2 do
      MainRecord.transaction do
        @user.highlights.attach fixture_file_upload("racecar.jpg")
        @user.highlights.attach fixture_file_upload("video.mp4")
      end
  ...
  test "attaching existing blobs to an existing record one at a time" do
    @user.highlights.attach create_main_blob(filename: "funky.jpg")
    @user.highlights.attach create_main_blob(filename: "town.jpg")
    assert_equal "funky.jpg", @user.highlights.first.filename.to_s
    assert_equal "town.jpg", @user.highlights.second.filename.to_s
  ...
  test "updating an existing record to attach existing blobs" do
    @user.update! highlights: [ create_main_file_blob(filename: "racecar.jpg"), create_main_file_blob(filename: "video.mp4") ]
    assert_equal "racecar.jpg", @user.highlights.first.filename.to_s
    assert_equal "video.mp4", @user.highlights.second.filename.to_s
  end

  test "updating an existing record to attach existing blobs from signed IDs" do
    @user.update! highlights: [ create_main_blob(filename: "funky.jpg").signed_id, create_main_blob(filename: "town.jpg").signed_id ]
    assert_equal "funky.jpg", @user.highlights.first.filename.to_s
    assert_equal "town.jpg", @user.highlights.second.filename.to_s
  end
  ...
  test "replacing existing, dependent attachments on an existing record via assign and attach" do
    [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ].tap do |old_blobs|
      ...
      perform_enqueued_jobs do
        @user.highlights.attach create_main_blob(filename: "whenever.jpg"), create_main_blob(filename: "wherever.jpg")
      end
      ...
      assert_not ActiveStorage::MainBlob.exists?(old_blobs.first.id)
      assert_not ActiveStorage::MainBlob.exists?(old_blobs.second.id)
      assert_not ActiveStorage::MainBlob.service.exist?(old_blobs.first.key)
      assert_not ActiveStorage::MainBlob.service.exist?(old_blobs.second.key)
    end
  end

  test "replacing existing, independent attachments on an existing record via assign and attach" do
    @user.vlogs.attach create_main_blob(filename: "funky.mp4"), create_main_blob(filename: "town.mp4")

    @user.vlogs = []
    assert_not @user.vlogs.attached?

    assert_no_enqueued_jobs only: ActiveStorage::PurgeJob do
      @user.vlogs.attach create_main_blob(filename: "whenever.mp4"), create_main_blob(filename: "wherever.mp4")
    end
  ...
  test "successfully updating an existing record to replace existing, dependent attachments" do
    [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ].tap do |old_blobs|
      ...
      perform_enqueued_jobs do
        @user.update! highlights: [ create_main_blob(filename: "whenever.jpg"), create_main_blob(filename: "wherever.jpg") ]
      end
  ...
  test "successfully updating an existing record to replace existing, independent attachments" do
    ...
    assert_no_enqueued_jobs only: ActiveStorage::PurgeJob do
      @user.update! vlogs: [ create_main_blob(filename: "whenever.mp4"), create_main_blob(filename: "wherever.mp4") ]
    end
  ...
  test "updating an existing record to attach one new blob and one previously-attached blob" do
    [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ].tap do |blobs|
  ...
  test "updating an existing record to remove independent attachments" do
    [ create_main_blob(filename: "funky.mp4"), create_main_blob(filename: "town.mp4") ].tap do |blobs|
  ...
  test "attaching an existing blob from a signed ID to a new record" do
    MainUser.new(name: "Jason").tap do |user|
      user.highlights.attach create_main_blob(filename: "funky.jpg").signed_id
      assert_predicate user, :new_record?
      assert_equal "funky.jpg", user.highlights.first.filename.to_s
  ...
  test "creating a record with existing blobs attached" do
    user = MainUser.create!(name: "Jason", highlights: [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ])
    assert_equal "funky.jpg", user.reload.highlights.first.filename.to_s
  ...
  test "creating a record with an existing blob from signed IDs attached" do
    user = MainUser.create!(name: "Jason", highlights: [
      create_main_blob(filename: "funky.jpg").signed_id, create_main_blob(filename: "town.jpg").signed_id ])
    assert_equal "funky.jpg", user.reload.highlights.first.filename.to_s
    assert_equal "town.jpg", user.reload.highlights.second.filename.to_s
  end

  test "creating a record with new blobs from uploaded files attached" do
    MainUser.new(name: "Jason", highlights: [ fixture_file_upload("racecar.jpg"), fixture_file_upload("video.mp4") ]).tap do |user|
      assert_predicate user, :new_record?
      assert_predicate user.highlights.first, :new_record?
      assert_predicate user.highlights.second, :new_record?
      assert_predicate user.highlights.first.blob, :new_record?
  ...
  test "creating a record with an unexpected object attached" do
    error = assert_raises(ArgumentError) { MainUser.create!(name: "Jason", highlights: :foo) }
  ...
  test "detaching" do
    [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ].tap do |blobs|
      ...
      assert_not @user.highlights.attached?
      assert ActiveStorage::MainBlob.exists?(blobs.first.id)
      assert ActiveStorage::MainBlob.exists?(blobs.second.id)
      assert ActiveStorage::MainBlob.service.exist?(blobs.first.key)
      assert ActiveStorage::MainBlob.service.exist?(blobs.second.key)
    end
  end
  ...
  test "purging" do
    [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ].tap do |blobs|
      ...
      assert_not @user.highlights.attached?
      assert_not ActiveStorage::MainBlob.exists?(blobs.first.id)
      assert_not ActiveStorage::MainBlob.exists?(blobs.second.id)
      assert_not ActiveStorage::MainBlob.service.exist?(blobs.first.key)
      assert_not ActiveStorage::MainBlob.service.exist?(blobs.second.key)
    end
  end

  test "purging attachment with shared blobs" do
    [
      create_main_blob(filename: "funky.jpg"),
      create_main_blob(filename: "town.jpg"),
      create_main_blob(filename: "worm.jpg")
    ].tap do |blobs|
      @user.highlights.attach blobs
      assert_predicate @user.highlights, :attached?
      ...
      assert_not ActiveStorage::MainBlob.exists?(blobs.first.id)
      assert ActiveStorage::MainBlob.exists?(blobs.second.id)
      assert ActiveStorage::MainBlob.exists?(blobs.third.id)

      assert_not ActiveStorage::MainBlob.service.exist?(blobs.first.key)
      assert ActiveStorage::MainBlob.service.exist?(blobs.second.key)
      assert ActiveStorage::MainBlob.service.exist?(blobs.third.key)
    end
  end
  ...
  test "duped record does not share attachment changes" do
    ...
    @user.dup.highlights.attach [ create_main_blob(filename: "town.mp4") ]
    assert_not_predicate @user, :changed_for_autosave?
  end

  test "clearing change on reload" do
    @user.highlights = [ create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg") ]
    assert_predicate @user.highlights, :attached?
  ...
  test "overriding attached reader" do
    ...
    begin
      ...
      MainUser.remove_method :highlights
    end
  end
  ...
  test "attaching blobs to a persisted, unchanged, and valid record, returns the attachments" do
    @user.highlights.attach create_main_blob(filename: "racecar.jpg")
    return_value = @user.highlights.attach create_main_blob(filename: "funky.jpg"), create_main_blob(filename: "town.jpg")
  ...
  test "raises error when misconfigured service is passed" do
    ...
    assert_match(/Cannot configure service :unknown for MainUser#featured_photos/, error.message)
  end
  ...
  test "raises error when unknown variant name is used to generate variant" do
    ...
    assert_match(/Cannot find variant :unknown for MainUser#highlights_with_variants/, error.message)
  end
  ...
  test "transforms variants later conditionally via proc" do
    assert_no_enqueued_jobs only: [ ActiveStorage::TransformJob, ActiveStorage::PreviewImageJob ] do
      @user.highlights_with_conditional_preprocessed.attach create_main_file_blob(filename: "racecar.jpg")
    end
  ...
  test "successfully attaches new blobs and destroys attachments marked for destruction via nested attributes" do
    town_blob = create_main_blob(filename: "town.jpg")
