...
class ActiveStorage::AnimalsBlobTest < ActiveSupport::TestCase
  ...
  test "unattached scope" do
    [ create_animals_blob(filename: "funky.jpg"), create_animals_blob(filename: "town.jpg") ].tap do |blobs|
      AnimalsUser.create! name: "DHH", avatar: blobs.first
      assert_includes ActiveStorage::AnimalsBlob.unattached, blobs.second
      assert_not_includes ActiveStorage::AnimalsBlob.unattached, blobs.first

      AnimalsUser.create! name: "Jason", avatar: blobs.second
      assert_not_includes ActiveStorage::AnimalsBlob.unattached, blobs.second
    end
  end

  test "create_and_upload does not permit a conflicting blob key to overwrite an existing object" do
    data = "First file"
    blob = create_animals_blob data: data

    assert_raises ActiveRecord::RecordNotUnique do
      ActiveStorage::AnimalsBlob.stub :generate_unique_secure_token, blob.key do
        create_animals_blob data: "This would overwrite"
      end
    end
  ...
  test "create_and_upload accepts a record for overrides" do
    assert_nothing_raised do
      create_animals_blob(record: AnimalsUser.new)
    end
  end

  test "create_and_upload raises for non-rewindable io" do
    assert_raises(ArgumentError) do
      ActiveStorage::AnimalsBlob.create_and_upload!(io: file_fixture("racecar.jpg"), filename: "racecar.jpg")
    end
  end

  test "record touched after analyze" do
    user = AnimalsUser.create!(
      name: "Nate",
      avatar: {
  ...
  test "build_after_unfurling generates a 28-character base36 key" do
    assert_match(/^[a-z0-9]{28}$/, build_main_blob_after_unfurling.key)
  end

  test "compose" do
    blobs = 3.times.map { create_animals_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false) }
    blob = ActiveStorage::AnimalsBlob.compose(blobs, filename: "all_numbers.txt")
  ...
  test "compose with unpersisted blobs" do
    blobs = 3.times.map { create_animals_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false).dup }

    error = assert_raises(ActiveRecord::RecordNotSaved) do
      ActiveStorage::AnimalsBlob.compose(blobs, filename: "all_numbers.txt")
    end
  ...
  test "compose with custom key" do
    blobs = 3.times.map { create_animals_blob(data: "123", filename: "numbers.txt", content_type: "text/plain", identify: false) }
    blob = ActiveStorage::AnimalsBlob.compose(blobs, key: "custom_key", filename: "all_numbers.txt")
  ...
  test "text?" do
    blob = create_animals_blob data: "Hello world!"
    assert_predicate blob, :text?
    assert_not_predicate blob, :audio?
  end

  test "download yields chunks" do
    blob   = create_animals_blob data: "a" * 5.0625.megabytes
    chunks = []
  ...
  test "open with integrity" do
    create_animals_file_blob(filename: "racecar.jpg").tap do |blob|
      blob.open do |file|
  ...
  test "open without integrity" do
    create_animals_blob(data: "Hello, world!").tap do |blob|
  ...
  test "open in a custom tmpdir" do
    create_animals_file_blob(filename: "racecar.jpg").open(tmpdir: tmpdir = Dir.mktmpdir) do |file|
      assert_predicate file, :binmode?
      assert_equal 0, file.pos
  ...
  test "URLs force content_type to binary and attachment as content disposition for content types served as binary" do
    blob = create_animals_blob(content_type: "text/html")
  ...
  test "URLs force attachment as content disposition when the content type is not allowed inline" do
    blob = create_animals_blob(content_type: "application/zip")
  ...
  test "URLs allow for custom filename" do
    blob = create_animals_blob(filename: "original.txt")
  ...
  test "purge does nothing when attachments exist" do
    create_animals_blob.tap do |blob|
      AnimalsUser.create! name: "DHH", avatar: blob
      assert_no_difference(-> { ActiveStorage::AnimalsBlob.count }) { blob.purge }
      assert ActiveStorage::AnimalsBlob.service.exist?(blob.key)
    end
  end

  test "purge doesn't raise when blob is not persisted" do
    build_main_blob_after_unfurling.tap do |blob|
  ...
  test "uses service from blob when provided" do
    with_animals_service("mirror") do
      blob = create_animals_blob(filename: "funky.jpg", service_name: :local)
  ...
  test "updating the content_type updates service metadata" do
    blob = directly_upload_animals_file_blob(filename: "racecar.jpg", content_type: "application/octet-stream")
  ...
  test "scope_for_strict_loading adds includes only when track_variants and strict_loading_by_default" do
    assert_empty ActiveStorage::AnimalsBlob.scope_for_strict_loading.includes_values

    with_strict_loading_by_default do
      assert_not_empty ActiveStorage::AnimalsBlob.scope_for_strict_loading.includes_values
  ...
  private
    def expected_url_for(blob, disposition: :attachment, filename: nil, content_type: nil, service_name: :local)
      ...
      "https://example.com/rails/active_storage/animals/disk/#{ActiveStorage.verifier.generate(key_params, expires_in: 5.minutes, purpose: :blob_key)}/#{filename}"
    end
end
