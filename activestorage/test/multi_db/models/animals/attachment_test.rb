...
class ActiveStorage::AnimalsAttachmentTest < ActiveSupport::TestCase
  ...
  teardown { ActiveStorage::AnimalsBlob.all.each(&:delete) }

  test "analyzing a directly-uploaded blob after attaching it" do
    blob = directly_upload_animals_file_blob(filename: "racecar.jpg")
  ...
  test "attaching a un-analyzable blob" do
    blob = create_animals_blob(filename: "blank.txt")
  ...
  test "attaching a blob doesn't touch the record" do
    ...
    blob = create_animals_blob_before_direct_upload byte_size: data.size, checksum: ActiveStorage.checksum_implementation.base64digest(data)
    blob.upload(io)
  ...
  test "mirroring a directly-uploaded blob after attaching it" do
    with_animals_service("mirror") do
      blob = directly_upload_animals_file_blob
      assert_not ActiveStorage::AnimalsBlob.service.mirrors.second.exist?(blob.key)

      perform_enqueued_jobs do
        @user.highlights.attach(blob)
      end

      assert ActiveStorage::AnimalsBlob.service.mirrors.second.exist?(blob.key)
    end
  end
  ...
  test "getting a signed blob ID from an attachment" do
    ...
    assert_equal blob, ActiveStorage::AnimalsBlob.find_signed!(signed_id)
    assert_equal blob, ActiveStorage::AnimalsBlob.find_signed(signed_id)
  end
  ...
  test "getting a signed blob ID from an attachment with a custom purpose" do
    ...
    assert_equal blob, ActiveStorage::AnimalsBlob.find_signed!(signed_id, purpose: :custom_purpose)
  end
  ...
  test "fail to find blob within expiration duration" do
    ...
    signed_id = @user.avatar.signed_id(expires_in: 1.minute)
    travel 2.minutes
    assert_nil ActiveStorage::AnimalsBlob.find_signed(signed_id)
  end
  ...
  test "signed blob ID backwards compatibility" do
    ...
    assert_equal blob, ActiveStorage::AnimalsBlob.find_signed!(signed_id_generated_old_way)
  end
  ...
  test "can create an attachment with the record having no attachment reflections" do
    assert_nothing_raised { ActiveStorage::AnimalsAttachment.create!(name: "whatever", record: @user, blob: create_animals_blob) }
  end
