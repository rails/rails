...
class ActiveStorage::AnimalsReflectionTest < ActiveSupport::TestCase
  test "reflecting on a singular attachment" do
    reflection = AnimalsUser.reflect_on_attachment(:avatar)
    assert_equal AnimalsUser, reflection.active_record
    ...
    reflection = AnimalsUser.reflect_on_attachment(:cover_photo)
    assert_equal :local, reflection.options[:service_name]

    reflection = AnimalsUser.reflect_on_attachment(:avatar_with_variants)
  ...
  test "reflection on a singular attachment with the same name as an attachment on another model" do
    reflection = AnimalsGroup.reflect_on_attachment(:avatar)
    assert_equal AnimalsGroup, reflection.active_record
  end

  test "reflecting on a collection attachment" do
    reflection = AnimalsUser.reflect_on_attachment(:highlights)
    assert_equal AnimalsUser, reflection.active_record
    assert_equal :highlights, reflection.name
    assert_equal :has_many_attached, reflection.macro
    assert_equal :purge_later, reflection.options[:dependent]

    reflection = AnimalsUser.reflect_on_attachment(:vlogs)
    assert_equal :local, reflection.options[:service_name]

    reflection = AnimalsUser.reflect_on_attachment(:highlights_with_variants)
    assert_instance_of Hash, reflection.named_variants
  end

  test "reflecting on all attachments" do
    reflections = AnimalsUser.reflect_on_all_attachments.sort_by(&:name)
    assert_equal [ AnimalsUser ], reflections.collect(&:active_record).uniq
