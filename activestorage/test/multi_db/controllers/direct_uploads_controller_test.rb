...
class ActiveStorage::DiskDirectUploadsControllerTest < ActionDispatch::IntegrationTest
  test "creating new main direct upload" do
    checksum = ActiveStorage.checksum_implementation.base64digest("Hello")
    metadata = {
      "foo" => "bar",
      "my_key_1" => "my_value_1",
    ...
    post rails_main_direct_uploads_url, params: { blob: {
      filename: "hello.txt", byte_size: 6, checksum: checksum, content_type: "text/plain", metadata: metadata } }

    response.parsed_body.tap do |details|
      assert_equal ActiveStorage::MainBlob.find(details["id"]), ActiveStorage::MainBlob.find_signed!(details["signed_id"])
      assert_equal "hello.txt", details["filename"]
      assert_equal 6, details["byte_size"]
      assert_equal checksum, details["checksum"]
      assert_equal metadata, details["metadata"]
      assert_equal "text/plain", details["content_type"]
      assert_match(/rails\/active_storage\/main\/disk/, details["direct_upload"]["url"])
      assert_equal({ "Content-Type" => "text/plain" }, details["direct_upload"]["headers"])
    end
  end

  test "creating new animals direct upload" do
    checksum = ActiveStorage.checksum_implementation.base64digest("Hello")
    metadata = {
      "foo" => "bar",
      "my_key_1" => "my_value_1",
    ...
    post rails_animals_direct_uploads_url, params: { blob: {
      filename: "hello.txt", byte_size: 6, checksum: checksum, content_type: "text/plain", metadata: metadata } }

    response.parsed_body.tap do |details|
      assert_equal ActiveStorage::AnimalsBlob.find(details["id"]), ActiveStorage::AnimalsBlob.find_signed!(details["signed_id"])
      assert_equal "hello.txt", details["filename"]
      assert_equal 6, details["byte_size"]
      assert_equal checksum, details["checksum"]
      assert_equal metadata, details["metadata"]
      assert_equal "text/plain", details["content_type"]
      assert_match(/rails\/active_storage\/animals\/disk/, details["direct_upload"]["url"])
      assert_equal({ "Content-Type" => "text/plain" }, details["direct_upload"]["headers"])
    end
  end

  test "creating new main direct upload does not include root in json" do
    checksum = ActiveStorage.checksum_implementation.base64digest("Hello")
    metadata = {
      "foo" => "bar",
      "my_key_1" => "my_value_1",
    ...
    set_include_root_in_json(true, MainRecord) do
      post rails_main_direct_uploads_url, params: { blob: {
        filename: "hello.txt", byte_size: 6, checksum: checksum, content_type: "text/plain", metadata: metadata } }
    end
  ...
  test "creating new animals direct upload does not include root in json" do
    checksum = ActiveStorage.checksum_implementation.base64digest("Hello")
    metadata = {
      "foo" => "bar",
      "my_key_1" => "my_value_1",
    ...
    set_include_root_in_json(true, AnimalsRecord) do
      post rails_animals_direct_uploads_url, params: { blob: {
        filename: "hello.txt", byte_size: 6, checksum: checksum, content_type: "text/plain", metadata: metadata } }
    end
  ...
  private
    def set_include_root_in_json(value, record_class)
      original = record_class.include_root_in_json
      record_class.include_root_in_json = value
      yield
    ensure
      record_class.include_root_in_json = original
    end
end
