...
class ActiveStorage::DiskControllerTest < ActionDispatch::IntegrationTest
  ...
  test "directly uploading blob without integrity" do
    data = "Something else entirely!"
    blob = create_blob_before_direct_upload byte_size: data.size, checksum: ActiveStorage::Checksum.for(ActiveStorage::Blob.service.checksum_algorithm).base64digest("bad data")
  ...
  test "directly uploading blob with different but equivalent content type" do
    ...
    blob = create_blob_before_direct_upload(
      byte_size: data.size, checksum: ActiveStorage::Checksum.for(ActiveStorage::Blob.service.checksum_algorithm).base64digest(data), content_type: "application/x-gzip")
  ...
  test "directly uploading blob with mismatched content length" do
    data = "Something else entirely!"
    blob = create_blob_before_direct_upload byte_size: data.size - 1, checksum: ActiveStorage::Checksum.for(ActiveStorage::Blob.service.checksum_algorithm).base64digest(data)
