...
class ActiveStorage::Blob < ActiveRecord::Base
  ...
  class << self
    ...
    def build_after_upload(io:, filename:, content_type: nil, metadata: nil, identify: true, service_name: nil, record: nil)
      new(filename: filename, content_type: content_type, metadata: metadata, service_name: service_name || default_service_name).tap do |blob|
        blob.upload(io, identify: identify)
      end
    end

    def build_after_unfurling(io:, filename:, content_type: nil, metadata: nil, identify: true, service_name: nil, record: nil) #:nodoc:
      new(filename: filename, content_type: content_type, metadata: metadata, service_name: service_name || default_service_name).tap do |blob|
    ...
    # When providing a content type, pass <tt>identify: false</tt> to bypass automatic content type inference.
    def create_after_upload!(io:, filename:, content_type: nil, metadata: nil, identify: true, service_name: nil, record: nil)
      build_after_upload(io: io, filename: filename, content_type: content_type, metadata: metadata, identify: identify, service_name: service_name || default_service_name).tap(&:save!)
    end
    ...
    # the signed ID.
    def create_before_direct_upload!(filename:, byte_size:, checksum:, content_type: nil, metadata: nil, service_name: nil, record: nil)
      create! filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type, metadata: metadata, service_name: service_name || default_service_name
    end
    ...
    attr_writer :default_service_name
  ...
  def service
    services[service_name] || services[self.class.default_service_name]
  end
