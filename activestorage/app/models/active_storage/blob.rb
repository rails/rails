...
# A blob is a record that contains the metadata about a file and a path/key for where that file resides on the service.
...
class ActiveStorage::Blob < ActiveStorage::Record
  ...
  class << self
    ...
    def build_after_unfurling(key: nil, io:, filename:, content_type: nil, metadata: nil, service_name: nil, identify: true, record: nil, path: nil) # :nodoc:
      new(key: key, filename: filename, content_type: content_type, metadata: metadata, service_name: service_name, path: path).tap do |blob|
        blob.unfurl(io, identify: identify)
      end
    end

    def create_after_unfurling!(key: nil, io:, filename:, content_type: nil, metadata: nil, service_name: nil, identify: true, record: nil, path: nil) # :nodoc:
      build_after_unfurling(key: key, io: io, filename: filename, content_type: content_type, metadata: metadata, service_name: service_name, identify: identify, path: path).tap(&:save!)
    end
    ...
    def create_and_upload!(key: nil, io:, filename:, content_type: nil, metadata: nil, service_name: nil, identify: true, record: nil, path: nil)
      create_after_unfurling!(key: key, io: io, filename: filename, content_type: content_type, metadata: metadata, service_name: service_name, identify: identify, path: path).tap do |blob|
        blob.upload_without_unfurling(io)
      end
    end

    # Returns a saved blob _without_ uploading a file to the service. This blob will point to a path where there is
    # no file yet. It's intended to be used together with a client-side upload, which will first create the blob
    # in order to produce the signed URL for uploading. This signed URL points to the path generated by the blob.
    # Once the form using the direct upload is submitted, the blob can be associated with the right record using
    # the signed ID.
    def create_before_direct_upload!(key: nil, filename:, byte_size:, checksum:, content_type: nil, metadata: nil, service_name: nil, record: nil, path: nil)
      create! key: key, filename: filename, byte_size: byte_size, checksum: checksum, content_type: content_type, metadata: metadata, service_name: service_name, path: nil
    end
    ...
    def compose(blobs, filename:, content_type: nil, metadata: nil)
      ...
      new(filename: filename, content_type: content_type, metadata: metadata, byte_size: blobs.sum(&:byte_size)).tap do |combined_blob|
        combined_blob.compose(blobs.map(&:path_on_service))
        combined_blob.save!
      end
    end
  end
  ...
  # Always refer to blobs using the signed_id or the path.
  def key
  ...
  def url(expires_in: ActiveStorage.service_urls_expire_in, disposition: :inline, filename: nil, **options)
    service.url path_on_service, expires_in: expires_in, filename: ActiveStorage::Filename.wrap(filename || self.filename),
  ...
  def service_url_for_direct_upload(expires_in: ActiveStorage.service_urls_expire_in)
    service.url_for_direct_upload path_on_service, expires_in: expires_in, content_type: content_type, content_length: byte_size, checksum: checksum, custom_metadata: custom_metadata
  end

  # Returns a Hash of headers for +service_url_for_direct_upload+ requests.
  def service_headers_for_direct_upload
    service.headers_for_direct_upload path_on_service, filename: filename, content_type: content_type, content_length: byte_size, checksum: checksum, custom_metadata: custom_metadata
  end
  ...
  # Uploads the +io+ to the service on the +path+ for this blob. Blobs are intended to be immutable, so you shouldn't be
  ...
  def upload_without_unfurling(io) # :nodoc:
    service.upload path_on_service, io, checksum: checksum, **service_metadata
  end

  def compose(keys) # :nodoc:
    self.composed = true
    service.compose(keys, path_on_service, **service_metadata)
  end
  ...
  def download_chunk(range)
    service.download_chunk path_on_service, range
  end
  ...
  def mirror_later # :nodoc:
    ActiveStorage::MirrorJob.perform_later(path_on_service, checksum: checksum) if service.respond_to?(:mirror)
  end
  ...
  def delete
    service.delete(path_on_service)

    if path?
      queue_variants_for_deletion if image?
    else
  ...
  private
    ...
    def update_service_metadata
      service.update_metadata path_on_service, **service_metadata if service_metadata.any?
    end

    def queue_variants_for_deletion
