...
class ActiveStorage::Blob < ActiveStorage::Record
  ...
  class << self
    ...
    # a signed upload URL. If a key is provided, a secure token will be appended automatically to ensure uniqueness.
    # After the upload, the blob can be attached to the corresponding record when the form is submitted.
    def create_before_direct_upload!(key: nil, filename:, byte_size:, checksum:, content_type: nil, metadata: nil, service_name: nil, record: nil)
      key = File.join(key, generate_unique_secure_token(length: MINIMUM_TOKEN_LENGTH)) if key.present?
