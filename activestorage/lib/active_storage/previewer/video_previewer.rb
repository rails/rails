  ...
  class Previewer::VideoPreviewer < Previewer
    ...
    private
      ...
      def probe_stream_codec_types(file)
        IO.popen([ self.class.ffprobe_path, "-v", "error", "-show_entries", "stream=codec_type", "-of", "csv=p=0",
          *Shellwords.split(ActiveStorage.ffprobe_arguments), file.path ], err: File::NULL) do |output|
          output.read.split("\n")
        end
      rescue Errno::ENOENT
