...
module ActiveStorage
  class Attached::Changes::CreateMany # :nodoc:
    ...
    def upload_sources
      pending_uploads.flat_map(&:upload_sources)
    end
    ...
    private
      ...
      def cleanup_new_records_after_failed_save(entries)
        entries.each do |entry|
          ...
          cleanup_record_after_failed_save(entry[:subchange].attachment, "attachment")
        end

        entries.each do |entry|
          next unless entry[:blob_was_new]

          cleanup_record_after_failed_save(entry[:subchange].blob, "blob")
        end
      end
