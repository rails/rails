...
module ActionCable
  module Socket
    class Base # :nodoc:
      attr_reader :server, :env, :connection, :logger, :worker_pool, :protocol
      delegate :event_loop, :pubsub, :config, to: :server

      def initialize(server, env, connection_class: nil, coder: ActiveSupport::JSON)
        @server, @env = server, env

        @worker_pool = server.worker_pool
        @logger = new_tagged_logger

        @websocket      = ActionCable::Socket::WebSocket.new(env, self, event_loop)
        @message_buffer = ActionCable::Socket::MessageBuffer.new(self)

        @_internal_subscriptions = nil
        @started_at = Time.now

        connection_class ||= config.connection_class.call
        @connection = connection_class.new(self, coder: coder)
      end
      ...
      def dispatch_websocket_message(websocket_message) #:nodoc:
        if websocket.alive?
          connection.handle_command websocket_message
        else
      ...
      def statistics
        {
          ...
          subscriptions: connection.subscriptions.identifiers,
