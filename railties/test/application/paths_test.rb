...
module ApplicationTests
  class PathsTest < ActiveSupport::TestCase
    ...
    def assert_in_load_path(*path)
      assert $LOAD_PATH.any? { |p| File.expand_path(p) == root(*path) }, "Load path does not include '#{root(*path)}'. They are:\n-----\n #{$LOAD_PATH.join("\n")}\n-----"
    end

    def assert_not_in_load_path(*path)
      assert !$LOAD_PATH.any? { |p| File.expand_path(p) == root(*path) }, "Load path includes '#{root(*path)}'. They are:\n-----\n #{$LOAD_PATH.join("\n")}\n-----"
    end
