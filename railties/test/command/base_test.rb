...
class Rails::Command::BaseTest < ActiveSupport::TestCase
  ...
  test "printing commands returns namespaced commands" do
    assert_equal %w(credentials:edit credentials:show credentials:diff credentials:fetch credentials:set), Rails::Command::CredentialsCommand.printing_commands.map(&:first)
