...
module ActiveSupport
  ...
  class MessageVerifier
    ...
    # legacy signed messages which are outdated messages generated on Rails 4.2 or 
    # earlier.
    #
    #   verifier = ActiveSupport::MessageVerifier.new 's3Krit'
    #
    ...
    #
    # Returns +nil+ if messages don't have the same purpose as that
    # given in +options+.
    #
    ...
    def verified(signed_message, options = {})
      if signed_message.include? '--'
        ...
        Claims.verify(load_claims(signed_message), options)
      end
    ...
    #
    # Raises +InvalidClaims+ if messages don't have the same purpose as that
    ...
    def verify(signed_message, options = {})
      if value = verified(signed_message, options)
        ...
        unless valid_message?(signed_message) && Claims.verify!(load_claims(signed_message), options)
    ...
    # message is signed as a JSON Web Token (JWT). The final message is the
    ...
    # object that is encoded in a JWS or JSON Web Encryprion (JWE), enabling the claims
    ...
    private
      ...
      def load_claims(signed_message)
