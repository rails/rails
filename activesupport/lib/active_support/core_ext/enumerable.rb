...
module Enumerable
  ...
  MANY_UNSET_PATTERN = Object.new
  private_constant :MANY_UNSET_PATTERN
  ...
  # Similar to <tt>Enumerable#any?</tt> in the standard library. Returns +true+ when at least two elements return a value other than +false+ or +nil+ for the provided block. <tt>{ |obj| obj }</tt> is used when no block is provided.
  #
  # If a pattern is supplied, the method returns whether <tt>pattern === element</tt> for at least two elements.
  #
  #   [nil, false].many?
  #   # => false
  #   [1, 2].many?
  #   # => true
  #   [Person.new(age: 30), Person.new(age: 42)].many? { |person| person.age > 26 }
  #   # => true
  #   [1, :b, 3.0].many?(Integer)
  #   # => false
  #
  def many?(pattern = MANY_UNSET_PATTERN, &block)
    block ||= -> obj { obj }
    if pattern != MANY_UNSET_PATTERN
      warn("given block not used", uplevel: 1) if block_given?
      block = -> obj { pattern === obj }
    end

    cnt = 0
    any? do |element|
      cnt += 1 if block.call(element)
      cnt > 1
    end
  end
