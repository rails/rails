...
class Class
  ...
  def update_heritable_value_of(attr, key, value, inheriting = false) # :nodoc:
    ...
    inherited_keys = if hashmap.instance_variable_defined?(:@_class_attribute_inherited_keys)
      hashmap.instance_variable_get(:@_class_attribute_inherited_keys)
    end

    if inheriting
      # if value is set but not inherited, do not inherit new value
