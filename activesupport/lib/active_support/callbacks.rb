    ...
    private
      ...
      module Conditionals # :nodoc: all
        class Value
          def initialize(&block)
            @block = Ractors.shareable_proc_attempt(block)
            freeze
          end
      ...
      class Callback # :nodoc:
        ...
        private
          ...
          def try_shareable_proc(object)
            object.is_a?(Proc) ? Ractors.shareable_proc_attempt(object) : object
          end
      ...
      module CallTemplate # :nodoc: all
        class MethodCall
          ...
          def make_lambda
            ...
            Ractors.shareable_proc_attempt do |target, value, &block|
      ...
      class CallbackChain # :nodoc:
        ...
        def initialize(name, config)
          ...
          @config[:terminator] = Ractors.shareable_proc_attempt(@config[:terminator]) if @config[:terminator].is_a?(Proc)
          @chain = []
          @all_callbacks = nil
          @single_callbacks = {}
          @mutex = Mutex.new
