...
module ActiveSupport
  ...
  class ContinuousIntegration
    ...
    #
    # Groups can be selectively run via the <tt>-g</tt>/<tt>--group</tt> CLI flag. Pass one or more
    # group names (comma-separated or repeated) to only run matching groups. Nested groups use
    # slash-separated paths (e.g. <tt>-g backend/unit</tt>). Selecting a parent group runs all its children.
    #
    # Examples:
    #
    ...
    #   $ bin/ci -g Checks
    #   $ bin/ci -g Tests
    #   $ bin/ci -g Checks,Tests
    #   $ bin/ci --group Checks --group Tests
    ...
    private
      ...
      def group_filtered?
        ...
        options[:groups].none? { |filter| current_path.start_with?(filter) || filter.start_with?(current_path) }
      end
