...
class NumericExtFormattingTest < ActiveSupport::TestCase
  ...
  def test_to_s__human_size
    ...
    assert_equal '1030 TiB',   terabytes(1026).to_s(:human_size)
    assert_equal '444 KiB',    kilobytes(444).to_s(:human_size)
    assert_equal '1020 MiB',   megabytes(1023).to_s(:human_size)
    assert_equal '3 TiB',      terabytes(3).to_s(:human_size)
    assert_equal '1.2 MiB',    1234567.to_s(:human_size, :precision => 2)
    assert_equal '3 Bytes',   3.14159265.to_s(:human_size, :precision => 4)
    assert_equal '1 KiB',      kilobytes(1.0123).to_s(:human_size, :precision => 2)
    assert_equal '1.01 KiB',   kilobytes(1.0100).to_s(:human_size, :precision => 4)
    assert_equal '10 KiB',     kilobytes(10.000).to_s(:human_size, :precision => 4)
  ...
  def test_to_s__human_size_with_options_hash
    ...
    assert_equal '1.0 KiB',   kilobytes(1.0123).to_s(:human_size, :precision => 2, :strip_insignificant_zeros => false)
    assert_equal '1.012 KiB', kilobytes(1.0123).to_s(:human_size, :precision => 3, :significant => false)
    assert_equal '1 KiB',     kilobytes(1.0123).to_s(:human_size, :precision => 0, :significant => true) #ignores significant it precision is 0
  end
  
  def test_to_s__human_size_with_custom_delimiter_and_separator
    assert_equal '1,01 KiB',     kilobytes(1.0123).to_s(:human_size, :precision => 3, :separator => ',')
    assert_equal '1,01 KiB',     kilobytes(1.0100).to_s(:human_size, :precision => 4, :separator => ',')
    assert_equal '1.000,1 TiB',  terabytes(1000.1).to_s(:human_size, :precision => 5, :delimiter => '.', :separator => ',')
  end
