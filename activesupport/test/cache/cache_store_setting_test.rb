...
class CacheStoreSettingTest < ActiveSupport::TestCase
  ...
  def test_mem_cache_fragment_cache_store
    assert_called_with(Dalli::Client, :new, [%w[localhost], { compress: false, serializer: JSON }]) do
  ...
  def test_mem_cache_fragment_cache_store_with_multiple_servers
    assert_called_with(Dalli::Client, :new, [%w[localhost 192.168.1.1], { compress: false, serializer: JSON }]) do
  ...
  def test_mem_cache_fragment_cache_store_with_options
    assert_called_with(Dalli::Client, :new, [%w[localhost 192.168.1.1], { timeout: 10, compress: false, serializer: JSON }]) do
