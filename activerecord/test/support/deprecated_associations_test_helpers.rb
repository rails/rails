module DeprecatedAssociationsTestHelpers
  private
    def assert_deprecated_association(association, &block)
      asserted = false
      mock = ->(reflection) do
      ...
      ActiveRecord::Associations::Deprecation.stub(:notify, mock, &block)
      assert asserted, "Expected a deprecation notification for #{@model}##{association}, but got none"
    end

    def assert_not_deprecated_association(association, &block)
      mock = ->(reflection) do
        if reflection.active_record == @model && reflection.name == association
