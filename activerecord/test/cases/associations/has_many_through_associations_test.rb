...
class HasManyThroughAssociationsTest < ActiveRecord::TestCase
  ...
  def test_has_many_through_on_new_record_follows_through_records_built_after_reading
    subscriber_1, subscriber_2 = Subscriber.create!(nick: "nick 1"), Subscriber.create!(nick: "nick 2")
    book = Book.new
    book.subscriptions.build(subscriber: subscriber_1)
    assert_equal [subscriber_1], book.subscribers.to_a

    book.subscriptions.build(subscriber: subscriber_2)
    assert_equal [subscriber_1, subscriber_2], book.subscribers.to_a
  end

  def test_has_many_through_on_new_record_counts_records_reached_through_through_records
    subscriber = Subscriber.create!(nick: "nick 1")
    book = Book.new
    book.subscriptions.build(subscriber: subscriber)

    assert_equal 1, book.subscribers.size
    assert_not_empty book.subscribers
    assert_includes book.subscribers, subscriber
    assert_equal [subscriber.nick], book.subscriber_ids
  end

  def test_has_many_through_on_new_record_ignores_through_records_marked_for_destruction
    subscriber_1, subscriber_2 = Subscriber.create!(nick: "nick 1"), Subscriber.create!(nick: "nick 2")
    book = Book.new
    book.subscriptions.build(subscriber: subscriber_1).mark_for_destruction
    book.subscriptions.build(subscriber: subscriber_2)

    assert_equal [subscriber_2], book.subscribers.to_a
  end

  def test_distinct_has_many_through_on_new_record_sets_distinct_through_records
    reference = Book.create!(name: "Reference")
    book = Book.new
    book.citations.build(reference_of: reference)
    book.citations.build(reference_of: reference)

    assert_equal [reference], book.references.to_a
  end

  def test_scoped_has_many_through_on_new_record_is_not_set_from_through_records
    post = Post.new(title: "Tagged", body: "Post")
    post.taggings.build(tag: Tag.create!(name: "Not misc"))

    assert_equal [], post.misc_tags.to_a
  end

  def test_replacing_has_many_through_on_new_record_keeps_through_records_built_by_hand
    member_1, member_2 = Member.create!(name: "one"), Member.create!(name: "two")
    club = Club.new
    club.memberships.build(member: member_1)
    club.members = [member_2]

    assert_equal [member_1, member_2], club.members.to_a

    club.save!
    assert_equal [member_1, member_2].sort, club.reload.members.sort
  end

  def test_adding_to_has_many_through_on_new_record_after_reading_creates_join_record
    member_1, member_2 = Member.create!(name: "one"), Member.create!(name: "two")
    club = Club.new
    club.memberships.build(member: member_1)
    club.members.to_a
