...
class HasOneThroughAssociationsTest < ActiveRecord::TestCase
  ...
  def test_setting_association_on_new_record_sets_nested_through_record_with_renamed_source
    category = Category.create!(name: "General")
    member = Member.new(current_membership: CurrentMembership.new(club: Club.create!(category: category)))
    member_detail = MemberDetail.new(member: member)

    assert_no_queries { assert_equal category, member_detail.member_club_category }
  end

  def test_setting_association_on_new_record_ignores_other_source_types
    club = Club.new
    club.sponsor = Sponsor.new(sponsorable: Club.create!)
    assert_nil club.sponsored_member

    member = Member.create!(name: "Groucho")
    club.sponsor = Sponsor.new(sponsorable: member)
    assert_equal member, club.sponsored_member
  end

  def test_association_on_new_record_follows_the_through_record
    club, other_club = Club.create!, Club.create!
    member = Member.new

    assert_nil member.club

    member.current_membership = CurrentMembership.new(club: club)
    assert_equal club, member.club

    member.current_membership = CurrentMembership.new(club: other_club)
    assert_equal other_club, member.club
  end

  def test_scoped_association_on_new_record_is_not_set_from_through_record
    member = Member.new
    member.membership = Membership.new(club: Club.create!, favorite: false)

    assert_nil member.favorite_club
  end

  def test_association_set_from_through_record_is_saved_by_its_own_parent
    member = Member.new
    member.current_membership = CurrentMembership.new(club: Club.new(name: "LRUG"))
    assert_equal "LRUG", member.club.name

    assert_difference -> { Club.count }, 1 do
      member.save!
    end
    assert_queries_count(1) { assert_equal "LRUG", member.club.name }
  end
