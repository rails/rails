...
class BelongsToWithDecoupledQueryConstraintsTest < ActiveRecord::TestCase
  ...
  def test_legacy_composite_foreign_key_targets_query_constrained_primary_key
    ...
    comment = Sharded::CompositePrimaryKeyComment.new(
      blog_id: expected_blog_post.blog_id,
      blog_post_id: expected_blog_post.id,
    )

    sql = capture_sql do
      assert_equal expected_blog_post, comment.blog_post
    end.first
