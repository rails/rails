...
class TestAutosaveAssociationTracksSavingState < ActiveRecord::TestCase
  test "@_save_cycle is incremented an decremented during multiple nested saves" do
    autosave_saving_stack = []

    ship_with_saving_stack = Class.new(Ship) do
      before_save { autosave_saving_stack << @_save_cycle }
      after_save  { autosave_saving_stack << @_save_cycle }
    end
    ...
    child.save!
    assert_equal [1, 1, 2, 2, 2, 2, 1, 1, 1, 1], autosave_saving_stack
    assert_equal 0, child.instance_variable_get(:@_save_cycle)
  end

  test "@_save_cycle and @_changes_applied are reset if validations fail" do
    child = Ship.new(name: "Nights Dirty Lightning")
    child.build_pirate
    assert_not child.save
    assert_equal 0, child.instance_variable_get(:@_save_cycle)
    assert_equal({}, child.instance_variable_get(:@_changes_applied))
  end
end
