  ...
  module ConnectionAdapters
    class RactorConnectionTest < ActiveRecord::TestCase
      ...
      if !in_memory_db? && RUBY_VERSION >= "4.0"
        ...
        def test_establish_connection_from_worker_with_config_object
          database_path = File.join(Dir.tmpdir, "ractor_connection_test_#{Process.pid}.sqlite3")
          selected, adapter_class_name = on_ractor(database_path) do |path|
            ...
            pool = ConnectionAdapters::RactorConnectionHandler.instance.establish_connection(config, owner_name: "RactorEstablishedBase")
            conn = pool.lease_connection
            value = conn.select_value("SELECT 7")
            pool.release_connection
            [value, pool.db_config.adapter_class.name]
          end
        ...
        def test_cold_model_entry_points_work_without_main_ractor_warmup
          expected = Topic.count

          result = on_ractor { ColdModel.count }
