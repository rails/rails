...
class DefaultBinaryTest < ActiveRecord::TestCase
  ...
  def test_default_varbinary_string
    if current_adapter?(:Mysql2Adapter)
      assert_nil DefaultBinary.new.varbinary_col
    else
  ...
  def test_default_varbinary_string_that_looks_like_hex
    if current_adapter?(:Mysql2Adapter)
      assert_nil DefaultBinary.new.varbinary_col_hex_looking
    else
      assert_equal "0xDEADBEEF", DefaultBinary.new.varbinary_col_hex_looking
    end
  end

  def test_default_varbinary_with_all_values
    if current_adapter?(:Mysql2Adapter)
      assert_nil DefaultBinary.new.varbinary_col_with_all_values
    else
