...
class FinderTest < ActiveRecord::TestCase
  ...
  def test_second_and_second_to_last_returns_the_right_data_even_without_loading_all_data_first
    assert_equal 2, owners(:loretta).pets.count, "sanity check"
    assert_equal ["pets.name desc"], owners(:loretta).pets.order_values

    owner = owners(:loretta)
    relation = owner.pets
    second_pet = relation.second
    second_to_last_pet = relation.second_to_last
    assert_not_equal second_pet.id, second_to_last_pet.id
  end

  def test_second_and_second_to_last_returns_the_right_data_when_preloading_all_data_first
    assert_equal 2, owners(:loretta).pets.count, "sanity check"
    assert_equal ["pets.name desc"], owners(:loretta).pets.order_values
