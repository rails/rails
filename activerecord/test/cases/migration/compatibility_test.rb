  ...
  class Migration
    class CompatibilityTest < ActiveRecord::TestCase
      ...
      def test_remove_foreign_key_on_8_0
        ...
        migrate = Class.new(ActiveRecord::Migration[8.0]) do
          def migrate(x)
            remove_foreign_key :sub_testings, :testings
            remove_foreign_key :sub_testings, :testings, column: :experiment_id
          end
        end.new

        assert_raise(StandardError, match: /Table 'sub_testings' has no foreign key for testings$/) {
          ActiveRecord::Migrator.new(:up, [migrate], @schema_migration, @internal_metadata).migrate
        }

        foreign_keys = @connection.foreign_keys("sub_testings")
        if %w[PostgreSQL SQLite].include?(@connection.adapter_name)
