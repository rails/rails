...
if ActiveRecord::Base.connection.supports_exclusion_constraints?
  ...
  module ActiveRecord
    class Migration
      class NoExclusionConstraintSupportTest < ActiveRecord::TestCase
        setup do
          @connection = ActiveRecord::Base.connection
        end
        ...
        def test_remove_exclusion_constraint_should_be_noop
          @connection.remove_exclusion_constraint :test_exclusion_constraints, name: "date_overlap"
        end

        def test_exclusion_constraints_should_raise_not_implemented
          assert_raises(NotImplementedError) do
            @connection.exclusion_constraints("test_exclusion_constraints")
          end
        end
      end
    end
