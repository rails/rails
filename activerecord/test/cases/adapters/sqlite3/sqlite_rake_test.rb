...
module ActiveRecord
  ...
  class SqliteDBDropTest < ActiveRecord::TestCase
    ...
    def test_does_not_disconnect_another_adapter_with_the_same_database_name
      other_config = ActiveRecord::DatabaseConfigurations::HashConfig.new(
        "arunit", "primary", adapter: "postgresql", database: @database
      )
      other_pool = Struct.new(:db_config) { def disconnect!; end }.new(other_config)

      ActiveRecord::Base.connection_handler.stub(:connection_pool_list, [other_pool]) do
        assert_not_called(other_pool, :disconnect!) do
          FileUtils.stub(:rm, nil) do
            ActiveRecord::Tasks::DatabaseTasks.drop @configuration, @root
          end
        end
