...
class MigrationTest < ActiveRecord::TestCase
  ...
  if ActiveRecord::Base.connection.supports_advisory_locks?
    ...
    def test_generate_migrator_advisory_lock_id
      ...
      assert_equal 2053462845, salt, "expected migration salt to remain equal to 2053462845, to maintain backward compatibility it cannot be changed"
      assert_equal expected_id, lock_id, "expected lock id generated by the migrator to be #{expected_id}, but it was #{lock_id} instead"
      assert_operator lock_id.bit_length, :<=, 63, "lock id must be a signed integer of max 63 bits magnitude"
    end
