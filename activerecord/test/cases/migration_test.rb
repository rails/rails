  ...
  if ActiveRecord::Base.connection.supports_advisory_locks?
    ...
    def test_generate_migrator_advisory_lock_id
      ...
      if current_adapter?(:PostgreSQLAdapter)
        ...
        expected_id = [Zlib.crc32(current_database), Zlib.crc32(current_schema)].pack("L*").unpack("l*")

        assert lock_id == expected_id, "expected lock id generated by the migrator to be #{expected_id}, but it was #{lock_id} instead"
        assert lock_id.is_a?(Array) && lock_id.all? { |id| id.is_a?(Integer) && id.bit_length <= 31 },
          "lock id must be an array of two signed integers of max 31 bits magnitude each"
      else
