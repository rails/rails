    ...
    class PostgreSQL < Arel::Visitors::ToSql
      private
        ...
        def prepare_update_statement(o)
          return super unless has_join_sources?(o) &&
            !has_limit_or_offset_or_orders?(o) && !has_group_by_and_having?(o)

          # When all join source nodes are {INNER | CROSS} JOIN nodes, move the
          # join conditions to the WHERE clause so they can reference the
          # updated table. Otherwise, alias the updated table, duplicate it into
          # the FROM clause, and add a self-join (which requires the primary key)
          if o.relation.right.all?(Arel::Nodes::InnerJoin)
            stmt = o.clone
            stmt.relation, stmt.wheres = o.relation.clone, o.wheres.clone
            stmt.relation.right = stmt.relation.right.map.with_index do |join, index|
              stmt.wheres << join.right.expr if join.right
              index == 0 ? join.left : Arel::Nodes::InnerJoin.new(join.left, nil)
            end
            stmt
          elsif o.key
            stmt = o.clone
