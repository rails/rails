    ...
    namespace :load do
      ActiveRecord::Tasks::DatabaseTasks.for_each(databases) do |name|
        ...
        task name => :load_config do
          original_db_config = ActiveRecord::Base.connection_db_config
          db_config = ActiveRecord::Base.configurations.configs_for(env_name: ActiveRecord::Tasks::DatabaseTasks.env, name: name)
          ActiveRecord::Tasks::DatabaseTasks.load_schema(db_config, ENV["SCHEMA"])
        ensure
          ActiveRecord::Base.establish_connection(original_db_config) if original_db_config
        end
      end
  ...
  namespace :test do
    ...
    task load_schema: %w(db:test:purge) do
      ...
      ActiveRecord::Base.configurations.configs_for(env_name: "test").each do |db_config|
        filename = ActiveRecord::Tasks::DatabaseTasks.dump_filename(db_config.name, db_config.schema_format)
        ActiveRecord::Tasks::DatabaseTasks.load_schema(db_config, filename)
      end
    ensure
      if should_reconnect
        ActiveRecord::Base.establish_connection(ActiveRecord::Tasks::DatabaseTasks.env.to_sym)
    ...
    ActiveRecord::Tasks::DatabaseTasks.for_each(databases) do |name|
      ...
      namespace :load_schema do
        task name => "db:test:purge:#{name}" do
          ...
          filename = ActiveRecord::Tasks::DatabaseTasks.dump_filename(name, db_config.schema_format)
          ActiveRecord::Tasks::DatabaseTasks.load_schema(db_config, filename)
        ensure
          if should_reconnect
            ActiveRecord::Base.establish_connection(ActiveRecord::Tasks::DatabaseTasks.env.to_sym)
