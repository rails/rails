  ...
  module SecureToken
    ...
    module ClassMethods
      ...
      def has_secure_token(attribute = :token, length: MINIMUM_TOKEN_LENGTH, on: ActiveRecord.generate_secure_token_on, prefix: nil)
        ...
        define_method("regenerate_#{attribute}") { update! attribute => self.class.generate_unique_secure_token(length: length, prefix: prefix) }
        set_callback on, on == :initialize ? :after : :before do
          if new_record? && !query_attribute(attribute)
            send("#{attribute}=", self.class.generate_unique_secure_token(length: length, prefix: prefix))
          end
        end
      end

      def generate_unique_secure_token(length: MINIMUM_TOKEN_LENGTH, prefix: nil)
        "#{prefix}#{SecureRandom.base58(length)}"
      end
    end
  end
end
