  ...
  class Migration
    ...
    def method_missing(method, *arguments, &block)
      say_with_time "#{method}(#{format_arguments(arguments)})" do
        normalize_table_name_arguments(method, arguments)
    ...
    private
      ...
      NON_TABLE_NAME_ARGUMENT_METHODS = [:execute, :enable_extension, :disable_extension, :quote].freeze
      private_constant :NON_TABLE_NAME_ARGUMENT_METHODS

      def normalize_table_name_arguments(method, arguments)
        # CommandRecorder records raw args inside revert;
        # the real adapter normalizes them on replay.
        return if reverting?

        return if arguments.empty?
        return if NON_TABLE_NAME_ARGUMENT_METHODS.include?(method)
