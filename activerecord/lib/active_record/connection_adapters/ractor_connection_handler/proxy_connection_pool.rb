...
module ActiveRecord
  module ConnectionAdapters
    class RactorConnectionHandler # :nodoc:
      class ProxyConnectionPool # :nodoc:
        include Proxy
        ...
        private
          ...
          # Checks a physical connection out of the main pool and pins it to
          # this Ractor; returns its token with the adapter profile the proxy
          # is built from.
          ...
          # Pins the main pool's connection identity. The pinned transaction,
          # if any, is begun by the caller on the worker-side proxy, so it
          # lives on the proxy's transaction manager and worker-side
          ...
          # The caller finishes the pinned transaction before releasing the
          # identity pin. The pin dies with its pool: when the named pool was
          # replaced or removed (pool token mismatch, see PoolConfig#pool_token),
