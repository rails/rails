  ...
  class Migration
    class CompatibilityBehavior # :nodoc:
      ...
      private
        ...
        # follow-up work after it. Dispatch is by method name, so a behavior
        # overriding an aliased operation such as `add_reference` must
        # re-declare its alias (`alias :add_belongs_to :add_reference`) or
        # calls through the aliased name bypass the override.
