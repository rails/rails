        ...
        @connection_subscriber = ActiveSupport::Notifications.subscribe("!connection.active_record") do |_, _, _, _, payload|
          ...
          if connection_name
            ...
            if pool
              ...
              @disable_transactions = config.respond_to?(:db_config) &&
                (use_transactional_tests && transaction_disabled_dbnames.include?(config.db_config.name.to_sym)) ||
                (!use_transactional_tests && !transaction_enabled_dbnames.include?(config.db_config.name.to_sym))
