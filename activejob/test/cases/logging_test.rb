...
class LoggingTest < ActiveSupport::TestCase
  ...
  def test_enqueue_log_level
    ...
    assert_no_match(/Enqueued HelloJob \(Job ID: .*?\) to .*?:.*Dummy/, @logger.messages)
  ...
  unless adapter_is?(:inline, :sneakers)
    def test_enqueue_at_log_level
      ...
      assert_no_match(/Enqueued HelloJob \(Job ID: .*\) to .*? at.*Cristian/, @logger.messages)
      assert_empty @logger.messages
  ...
  def test_enqueue_all_log_level
    @logger.level = WARN
    ActiveJob.perform_all_later(LoggingJob.new("Dummy"), HelloJob.new("Jamie"), HelloJob.new("John"))
    assert_no_match(/Enqueued 3 jobs to .+ \(2 HelloJob, 1 LoggingJob\)/, @logger.messages)
    assert_empty @logger.messages
  ...
  def test_perform_start_log_level
    ...
    assert_no_match(/Performing LoggingJob \(Job ID: .*?\) from .*? with arguments:.*Dummy/, @logger.messages)
    assert_empty @logger.messages
  ...
  unless adapter_is?(:inline, :sneakers)
    def test_enqueue_retry_log_level
      ...
      assert_no_match(/Retrying RetryJob \(Job ID: .*?\) after \d+ attempts in 3 seconds, due to a DefaultsError.*\./, @logger.messages)
      assert_empty @logger.messages
  ...
  def test_enqueue_retry_log_level_on_retry_job
    @logger.level = WARN
    perform_enqueued_jobs { RescueJob.perform_later "david" }
    assert_no_match(/Retrying RescueJob \(Job ID: .*?\) after \d+ attempts in 0 seconds\./, @logger.messages)
    assert_empty @logger.messages
  ...
  unless adapter_is?(:inline, :sneakers)
    def test_retry_stopped_log_level
      ...
      assert_no_match(/Stopped retrying RetryJob \(Job ID: .*?\) due to a CustomCatchError.*, which reoccurred on \d+ attempts\./, @logger.messages)
      assert_empty @logger.messages
  ...
  def test_discard_log_level
    ...
    assert_no_match(/Discarded RetryJob \(Job ID: .*?\) due to a DiscardableError.*\./, @logger.messages)
    assert_empty @logger.messages
