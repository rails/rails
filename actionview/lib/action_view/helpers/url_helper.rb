...
module ActionView
  module Helpers # :nodoc:
    ...
    module UrlHelper
      ...
      #          <strong>Email me:</strong> <span>me@domain.com</span>
      #        </a>
      def mail_to(recipients, name = nil, html_options = nil, cc: nil, bcc: nil, reply_to: nil, subject: nil, body: nil, **kwrest, &block)
        html_options, name = name, nil if name.is_a?(Hash)
        html_options = kwrest.merge(html_options || {}).stringify_keys

        cc       = Array(cc).map       { |addr| ERB::Util.url_encode(addr) }.join(",")
        bcc      = Array(bcc).map      { |addr| ERB::Util.url_encode(addr) }.join(",")
        reply_to = Array(reply_to).map { |addr| ERB::Util.url_encode(addr) }.join(",")

        extras = []
        extras << "cc=#{cc}"                                 if cc.present?
        extras << "bcc=#{bcc}"                               if bcc.present?
        extras << "body=#{ERB::Util.url_encode(body)}"       if body.present?
        extras << "subject=#{ERB::Util.url_encode(subject)}" if subject.present?
        extras << "reply-to=#{reply_to}"                     if reply_to.present?
        extras = extras.empty? ? "" : "?" + extras.join("&")

        recipients = Array(recipients)
        encoded_email_addresses = recipients.map { |addr| ERB::Util.url_encode(addr) }.join(",").gsub("%40", "@")
        html_options["href"] = "mailto:#{encoded_email_addresses}#{extras}"
