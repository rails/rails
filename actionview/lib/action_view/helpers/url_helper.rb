...
module ActionView
  module Helpers # :nodoc:
    ...
    module UrlHelper
      ...
      #          <strong>Email me:</strong> <span>me@domain.com</span>
      #        </a>
      def mail_to(email_address, name = nil, html_options = nil, cc: nil, bcc: nil, reply_to: nil, subject: nil, body: nil, **kwrest, &block)
        html_options, name = name, nil if name.is_a?(Hash)
        html_options = kwrest.merge(html_options || {}).stringify_keys

        extras = []
        extras << "cc=#{ERB::Util.url_encode(cc)}"             if cc.present?
        extras << "bcc=#{ERB::Util.url_encode(bcc)}"           if bcc.present?
        extras << "body=#{ERB::Util.url_encode(body)}"         if body.present?
        extras << "subject=#{ERB::Util.url_encode(subject)}"   if subject.present?
        extras << "reply-to=#{ERB::Util.url_encode(reply_to)}" if reply_to.present?
