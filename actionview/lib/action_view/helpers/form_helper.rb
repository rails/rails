...
module ActionView
  ...
  module Helpers
    ...
    module FormHelper
      ...
      # === Nested Attributes Examples
      #
      # When the current scope's object has a nested attribute writer —
      # most likely through +accepts_nested_attributes_for+ — for an
      # attribute, +fields+ yields a new scope for that attribute. Useful when
      # setting attributes on a parent and its associations in one go.
      #
      ...
      #   end
      #
      # This model can now be used with a nested +fields+, like so:
      #
      ...
      #   end
      #
      # To destroy the associated model through the form, you have
      # to enable it using <tt>:allow_destroy</tt>:
      #
      #   class Person < ActiveRecord::Base
      #     has_one :address
      #     accepts_nested_attributes_for :address, allow_destroy: true
      ...
      #
      # Now, when you use a form element with the <tt>_destroy</tt> parameter,
      # with a value that evaluates to +true+, the associated model is destroyed
      # (eg. 1, '1', true, or 'true'):
      #
      #   <%= form_with model: @person do |person_form| %>
      #     ...
      #     <%= person_form.fields :address do |address_fields| %>
      #       ...
      #       Delete: <%= address_fields.check_box :_destroy %>
      #     <% end %>
      #     ...
      ...
      #
      # Note that the <tt>projects_attributes=</tt> writer method is in fact
      # required for +fields+ to correctly identify <tt>:projects</tt> as a
      # collection, and the correct indices to be set in the form markup.
      #
      # When projects is already an association on Person you can use
      # +accepts_nested_attributes_for+ to define the writer method for you:
      ...
      #   end
      #
      # This model can now be used with a nested +fields+. The block given to
      # the nested +fields+ call will be repeated for each instance in the
      # collection:
      #
      #   <%= form_with model: @person do |person_form| %>
      #     ...
      #     <%= person_form.fields :projects do |project_fields| %>
      #       <% if project_fields.object.active? %>
      #         Name: <%= project_fields.text_field :name %>
      #       <% end %>
      #     <% end %>
      ...
      #     <% @person.projects.each do |project| %>
      #       <% if project.active? %>
      #         <%= person_form.fields :projects, model: project do |project_fields| %>
      #           Name: <%= project_fields.text_field :name %>
      #         <% end %>
      #       <% end %>
      #     <% end %>
      ...
      #   <%= form_with model: @person do |person_form| %>
      #     ...
      #     <%= person_form.fields :projects, model: @active_projects do |project_fields| %>
      #       Name: <%= project_fields.text_field :name %>
      #     <% end %>
      #     ...
      #   <% end %>
      #
      # To destroy any of the associated models through the
      # form, you have to enable it using <tt>:allow_destroy</tt>:
      #
      #   class Person < ActiveRecord::Base
      #     has_many :projects
      #     accepts_nested_attributes_for :projects, allow_destroy: true
      ...
      #   <% end %>
      #
      # When collections is used you might want to know the index of each
      # object into the array. Use the FormBuilder's <tt>index</tt> method:
      #
      #   <%= form_with model: @person do |person_form| %>
      #     ...
      #     <%= person_form.fields :projects do |project_fields| %>
