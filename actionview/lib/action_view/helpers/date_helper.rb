  ...
  module Helpers # :nodoc:
    ...
    module DateHelper
      ...
      def relative_time_in_words(from_time, options = {})
        options = { scope: :'datetime.relative' }.merge(options)
        time = time_ago_in_words(from_time, options.except(:scope))
        key = from_time > Time.now ? :future : :past

        I18n.t(key, time: time, scope: options[:scope], locale: options[:locale])
      end
