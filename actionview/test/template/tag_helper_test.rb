...
class TagHelperTest < ActionView::TestCase
  ...
  def test_token_list_and_class_names
    [:token_list, :class_names].each do |helper_method|
      ...
      assert_equal "song play", helper.(["song", { ["play", "pause"] => true }])
      assert_equal "song pause", helper.(["song", { ["play", "pause"] => false }])
      assert_equal "song play", helper.({ "song": true, ["play", "pause"] => true })
      assert_equal "song pause", helper.({ "song": true, ["play", "pause"] => false })
      assert_equal "song play", helper.([{ "song": true }, { ["play", "pause"] => true }])
      assert_equal "song pause", helper.([{ "song": true }, { ["play", "pause"] => false }])
      assert_equal "song play", helper.({ song: true, [:play, :pause] => true })
      assert_equal "song pause", helper.({ song: true, [:play, :pause] => false })
