...
class PrecompilerEndToEndTest < ActiveSupport::TestCase
  # Minimal stand-in for a Rails::Engine's `paths[key].existent` API used by
  # ActionView::Precompiler. Only the three keys the precompiler reads are
  # populated.
  class FakePaths
    ...
    def [](key)
      Existent.new(Array(@mapping[key]))
    end

    Existent = Struct.new(:existent)
  end
  ...
  def setup
    ...
    @tmpdir = Dir.mktmpdir("precompiler_end_to_end")
    @views_dir = File.join(@tmpdir, "views")
    @controllers_dir = File.join(@tmpdir, "controllers")
    @helpers_dir = File.join(@tmpdir, "helpers")
    [@views_dir, File.join(@views_dir, "posts"), File.join(@views_dir, "comments"),
     @controllers_dir, @helpers_dir].each do |d|
      FileUtils.mkdir_p(d)
    end

    # Templates. Some are referenced by render calls in the controller/helper
    # source, others are implicit action renders discovered from
    # `controller.action_methods`.
    File.write(File.join(@views_dir, "posts/index.html.erb"),
      %(<h1>Posts</h1>\n<%= render partial: "posts/summary", locals: { post: @post } %>))
    File.write(File.join(@views_dir, "posts/show.html.erb"),
      %(<h1><%= @post %></h1>\n<%= render "comments/comment", comment: @comment %>))
    File.write(File.join(@views_dir, "posts/_summary.html.erb"), "<p>summary</p>")
    File.write(File.join(@views_dir, "comments/_comment.html.erb"), "<p>comment</p>")

    # Controller source — the scanner picks up the render call to
    # `posts/summary` via a helper-invoked path, and `action_methods` yields
    # the implicit renders for index/show.
    ...
    # Helper source with a render call the helper scanner should pick up.
    File.write(File.join(@helpers_dir, "posts_helper.rb"), <<~RUBY)
      module PostsHelper
        def comment_widget(comment)
    ...
    @controller_class.view_paths = ActionView::PathSet.new(
      [ActionView::FileSystemResolver.new(@views_dir)]
    )

    @engine = FakeEngine.new(FakePaths.new(
      "app/views" => @views_dir,
  ...
  def test_precompile_compiles_expected_templates_and_emits_notification
    compiled_virtual_paths = []
    compile_subscriber = ->(_name, _start, _finish, _id, payload) do
      compiled_virtual_paths << payload[:virtual_path]
    end

    precompile_payload = nil
    precompile_subscriber = ->(_name, _start, _finish, _id, payload) do
      precompile_payload = payload
    end

    ActiveSupport::Notifications.subscribed(compile_subscriber, "!compile_template.action_view") do
      ActiveSupport::Notifications.subscribed(precompile_subscriber, "precompile_templates.action_view") do
    ...
    # Every template referenced by scanners or implicit action renders should
    # have been compiled: the implicit action renders (index/show) plus the
    # partials the controller/helper scanners discovered
    # (posts/_summary, comments/_comment).
    expected = %w[
      posts/index
      posts/show
      posts/_summary
    ...
    unique_compiled = compiled_virtual_paths.uniq
    expected.each do |vp|
      assert_includes unique_compiled, vp, "expected #{vp} to be compiled, got #{unique_compiled.inspect}"
    end

    # Payload should report the count reported through the precompile
    # instrumentation, and it should match the number of compile events we
    # observed.
    assert_not_nil precompile_payload, "precompile_templates.action_view was not instrumented"
    assert_kind_of Integer, precompile_payload[:count]
    assert_operator precompile_payload[:count], :>=, expected.size
    assert_equal precompile_payload[:count], compiled_virtual_paths.size
  end
end
