...
Alternatively, you can customise the `I18n.exception_handler`. See the [i18n
...
behavior, or replace the old values with the corresponding new ones (`true`
replaces `:all`, `false` replaces `:none`).
...
Constants managed by the `once` autoloader can be autoloaded during
