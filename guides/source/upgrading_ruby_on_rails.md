...
### Since 5.1 migrations have to be versioned

In 5.0 when you generated migration it would get a version (the migration class
would inherit from `ActiveRecord::Migration[5.0]`). But if we the migration didn't have
version specified we assumed that it is the migration generated before Rails 5.

The versioning has been introduced so that it is possible to improve the SQL code that
is generated by migration gradually, but at the same time making sure that old transactions
would result in the same query as when they were first introduced.

From 5.1 migration are required so if your Migration inherits from `ActiveRecord::Migration`
we raise an error saying "Directly inheriting from ActiveRecord::Migration is not supported".
So when upgrading from 5.0 to 5.1 you may want to change your migrations so that they inherit
 from `ActiveRecord::Migration[4.2]` to be able to run migrations created before 5.0.
