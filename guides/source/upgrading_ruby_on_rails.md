...
Overwrite /myapp/config/application.rb? (enter "h" for help) [Ynaqdh]
      ...
      create  config/application/new_framework_defaults_7_1.rb
...
```
...
To allow you to upgrade to new defaults one by one, the update task has created a file `config/application/new_framework_defaults_X.Y.rb` (with the desired Rails version in the filename). You should enable the new configuration defaults by uncommenting them in the file; this can be done gradually over several deployments. Once your application is ready to run with new defaults, you can remove this file and flip the `config.load_defaults` value.
