...
Can also be set to a callable (such as a lambda), which is invoked on each read so the
expiry can be computed dynamically, for example per request in multi-tenant applications:
...
```ruby
config.active_storage.urls_expire_in = -> { Current.tenant&.url_expiry || 1.day }
```
