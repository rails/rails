...
#### `config.active_record.schema_cache_dump_order`

Controls the order of columns and indexes in schema cache dump files. When set
to `:database` (the default), columns and indexes are written in the order
returned by the database. When set to `:sorted`, they are sorted alphabetically,
which produces deterministic output suitable for use in cache keys or digests.
