...
Directs ActiveStorage::Attachments to touch its corresponding record when updated.
The default is true. This can be overridden on a per-attachment basis using the
`touch:` option on `has_one_attached` or `has_many_attached`:
...
Note: The global setting takes precedence - if set to `false`, attachments will
never touch their parent records regardless of the per-attachment setting.
