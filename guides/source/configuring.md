...
Controls whether ActiveStorage::Attachments and their corresponding records are touched when a Blob is updated. The default is true so both attachments and records are updated. When set to false, neither the attachments nor the records will be touched when a Blob is updated.
