...
You can see what the engine has so far by running the migration generated by the 
scaffold generator and the test/dummy rails application.
In order to do so you have to add a server to the engine's Gemfile.
For example add `gem puma` and run `bundle install`. Then
...
When you open `http://localhost:3000/blorgh/articles` you will see the default scaffold that has
