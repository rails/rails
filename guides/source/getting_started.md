...
Rails is saying that a template is missing for the new action. Generally,
it is the `articles/new` template matching with the controller and action names.
Rails will first look for this template. If not found, it will attempt to load
a template called `application/new`. It looks for one here because the `ArticlesController`
...
we don't specify how the response should be. We just added the `create` action
but didn't specify anything about how the response should be. In this case, what the
