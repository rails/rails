...
This looks quite similar to the migration for `Product`, the main new thing is `belongs_to` which adds a `product_id` foreign key column. This sets up the database side of a one-to-many relationship, where each Subscriber "belongs to" a single Product instance. 
