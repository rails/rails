...
TIP: You may wish to use `p` or `pp` instead of `puts` to log debug
statements. `p` prints the raw object passed to it by calling `inspect`
on it, rather than `to_s` which is what `puts` calls. It also returns
the supplied object, contrasting with `puts` which always returns `nil`.
<br><br>
`pp` offers better log formatting than `p` by _pretty printing_ objects such
as hashes and arrays in a more human-readable format.
