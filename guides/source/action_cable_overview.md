...
You can specify a different URL to connect to:

```js
createConsumer('https://ws.example.com/cable')
```

Or, use a function to dynamically generate the URL:
...
In addition to the required parameter `channel` you can pass extra parameters
in the first argument to `subscriptions.create`. These parameters will become the
params hash in the cable channel. For example, a consumer could subscribe to a
specific chat room only:
