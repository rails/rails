...
be set to `:sql` or `:sql_filesystem_versions`.
...
#### Using the `:sql_filesystem_versions` schema dumper

The `:sql_filesystem_versions` schema format is similar to `:sql` but addresses a common issue that teams face when working with SQL schema dumps in version control: merge conflicts in the schema migrations table section.

This format stores the main schema structure in `db/structure.sql` like the `:sql` format, but it keeps migration versions in separate files within a `db/schema_migrations` directory (one file per version). When loading the schema, both the structure and migration versions are combined seamlessly.

To use this format, set the configuration in your `config/application.rb`:

```ruby
config.active_record.schema_format = :sql_filesystem_versions
```

You can configure the path where the migration files are stored by setting the `schema_migrations_path` option in your database configuration:

```yaml
production:
  adapter: postgresql
  database: myapp_production
  schema_migrations_path: db/custom_migrations_path
```
