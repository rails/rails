...
dictionary, or similar. See section [Using different backends](#using-different-backends)
below.
...
Rails also provides [`relative_time_in_words`](https://api.rubyonrails.org/classes/ActionView/Helpers/DateHelper.html#method-i-relative_time_in_words) when you want a localized
...
You can also set the locale from the _subdomain_ in a very similar way. For
example, `es.example.com` is often easier to configure and operate than a
separate top-level domain for each locale.
