...
The **translations load path** (`I18n.load_path`) is an array of paths to files that will be loaded automatically. Configuring this path allows for customization of translation directory structure and file naming scheme.
...
This example illustrates using a URL query parameter to set the locale (e.g. `http://example.com/books?locale=pt`). With this approach, `http://localhost:3000?locale=pt` renders the Portuguese localization, while `http://localhost:3000?locale=de` loads a German localization.
...
There are two strings in our code that are English that users will be rendered in our response ("Hello Flash" and "Hello World"). In order to internationalize this code, these strings need to be replaced by calls to Rails' `#t` helper with an appropriate key for each string:
...
One key consideration for successfully internationalizing an application is to avoid making incorrect assumptions about grammar rules when abstracting localized code. Grammar rules that seem fundamental in one locale may not hold true in another locale.

Improper abstraction is shown in the following example, where assumptions are made about punctuation type, punctuation placement, and the ordering of different parts of speech.

```erb
# app/views/home/index.html.erb
<%= "#{t('greeting')}, #{current_user.name}!" %>
```
...
# config/locales/different.yml
different:
  greeting: "hello"
```

If a different language's proper translation were `Bill, welcome` instead of `Welcome, Bill!`, the abstraction cannot give a proper translation.

```ruby
def translated_greeting
  "#{t('greeting')}, #{current_user.name}!"
end
I18n.locale = :en
# Desired Translation: "Hello, Bill!"
translated_greeting # => "Hello, Bill!"

I18n.locale = :different
# Desired Translation: "Bill, hello"
translated_greeting # => "hello, Bill!"
```

To create proper abstraction, use variables in translation definitions and pass the values for these variables to the translation method.

Proper abstraction is shown in the following example.

```erb
# app/views/home/index.html.erb
<%= t('greet_username', user: current_user.name) %>
```
...
# config/locales/different.yml
different:
  greet_username: "%{user}, hello"
```

All grammatical decisions are made in the translation itself, so the abstraction can give a proper translation.
...
I18n.locale = :different
# Desired Translation: "Bill, hello"
translated_greeting # => "Bill, hello"
```


See the section on [Interpolation](#interpolation) for more information on variable interpolation with the I18n module.
