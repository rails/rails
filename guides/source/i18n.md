...
If you specify a locale via the `locale` argument for the `mail` method, you need to pass this to `default_i18n_subject` as well. This is not necessary, if the locale is specified using the `default` hash or `I18n.locale`.
...
class UserMailer < ActionMailer::Base
  def welcome(user)
    mail(
      to: user.email,
      locale: user.locale,
      subject: default_i18n_subject(user: user.name, locale: user.locale)
    )
  end
end
```
