...
TIP: Use the `--pretend` option with generators to see the difference between various generator commands before actually running them. This can be a handy to see what files will be generated by the `model` vs. `resource` vs. `scaffold` generators, for example.
