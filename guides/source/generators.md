...
You will get a list of all available generators along with usage instructions. For example, you can run a generator with the `--pretend` (or `-p`) option to get a sense of what a specific generator would do *without* actually making the changes:
...
TIP: The `--pretend` option can be handy to check the difference in files generated by related generators before actually running them. For example, `model` vs. `resource` generator.
...
which provides useful options for parsing and an API for manipulating
files.

The first step in manually creating a generator is to create a file named `initializer_generator.rb` inside `lib/generators` with the following content:
...
NOTE: `$LOAD_PATH` is initialized by Ruby and expanded by Bundler and Rails at boot time. Bundler adds each gem's `lib/` directory, and Rails adds your application's `lib/` directory, which is why generators placed there are findable. You can inspect the full load path by running `bin/rails runner 'puts $LOAD_PATH'`. You can modify it with `config.autoload_paths` in your `application.rb` file.
