...
`pluralize` can also take an optional `count` parameter. If `count == 1` the singular form will be returned. For any other positive integer value of `count` the plural form will be returned:
...
If `count` is an object that responds to `count`, such as an array, the object's count will be used as the count value:

```ruby
dudes = %w[John]
"dude".pluralize(dudes) # => "dude"

dudes = %w[John Jim Joe]
"dude".pluralize(dudes) # => "dudes"
```

Active Record uses the `pluralize` method to compute the default table name that corresponds to a model:
