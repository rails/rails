...
Sometimes it is helpful to cache the result of a method after it has been called. The option `:memoize` memoizes the method generated by `delegate`:

```ruby
delegate :raw_data, to: :data_reader, memoize: true

# the first method call will call through to data_reader
raw_data

# the second will return the return value of the first call
raw_data
```
