...
even if the rest of your application requires authentication. `before_action`s
in your own controllers (such as requiring a logged-in user) do not apply to
Active Storage’s built-in controllers.
...
unattached records using the [unattached scope][Blob.unattached]. Below is an
example using a [custom rake task](command_line.html#custom-rake-tasks).

[Blob.unattached]:
https://api.rubyonrails.org/classes/ActiveStorage/Blob.html#method-c-unattached
...
uploads](#purging-unattached-uploads).
